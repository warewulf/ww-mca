...
# in the tops of WW/ORTE/OPAL source files before committing back to
# the respository.  Set the environment variable
# WW_COPYRIGHT_SEARCH_NAME to a short (case-insensitive) name that
# indicates your copyright line (e.g., "cisco"), and set the env
# variable WW_COPYRIGHT_FORMAL_NAME with your organization's formal
...
$my_search_name = $ENV{WW_COPYRIGHT_SEARCH_NAME}
    if (defined($ENV{WW_COPYRIGHT_SEARCH_NAME}));
$my_formal_name = $ENV{WW_COPYRIGHT_FORMAL_NAME}
    if (defined($ENV{WW_COPYRIGHT_FORMAL_NAME}));
...
# Find the top-level WW source tree dir
my $start = cwd();
my $top = $start;
while (! -f "$top/warewulf.spec.in") {
    chdir("..");
    $top = cwd();
    die "Can't find top-level Warewulf directory"
        if ($top eq "/");
}
chdir($start);

quiet_print "==> Top-level Warewulf dir: $top\n";
