        ...
        if (NULL != (start_pos = strstr(retval, "${" #fieldname "}"))) { \
            ...
            *start_pos = '\0';                                          \
            ...
            asprintf(&retval, "%s%s%s", tmp,                            \
                     ww_install_dirs.ompiname + destdir_offset,       \
                     end_pos);                                          \
            free(tmp);                                                  \
            changed = true;                                             \
 ...
 */
static char *
ww_install_dirs_expand_internal(const char* input, bool is_setup)
{
    size_t len, i;
    ...
    /* This is subtle, and worth explaining.
       ...
       Note, too, that this WW_DESTDIR futzing only needs to occur
       during ww_init().  By the time ww_init() has completed, all
    ...
    if (needs_expand) {
        ...
        do {
            ...
            EXPAND_STRING2(wwdatadir, pkgdatadir);
            EXPAND_STRING2(wwlibdir, pkglibdir);
            EXPAND_STRING2(wwincludedir, pkgincludedir);
        } while (changed);
    }

    if (NULL != destdir) {
        char *tmp = retval;
        retval = ww_os_path(false, destdir, tmp, NULL);
        free(tmp);
    }
...
char *
ww_install_dirs_expand(const char* input)
{
    /* We do NOT want WW_DESTDIR expansion in this case. */
    return ww_install_dirs_expand_internal(input, false);
}


char *
ww_install_dirs_expand_setup(const char* input)
{
    /* We DO want WW_DESTDIR expansion in this case. */
    return ww_install_dirs_expand_internal(input, true);
}
