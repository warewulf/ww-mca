                       ...
                       ww_dl_handle_t **handle, char **err_msg)
{
    ...
    *handle = NULL;
    ...
    return (NULL != local_handle) ? WW_SUCCESS : WW_ERROR;
}


static int dlopen_lookup(ww_dl_handle_t *handle, const char *symbol,
...
static int dlopen_close(ww_dl_handle_t *handle)
{
    assert(handle);
                              ...
                              void *data)
{
    ...
    dirs = ww_argv_split(search_path, WW_ENV_SEP);
    for (int i = 0; NULL != dirs && NULL != dirs[i]; ++i) {
        ...
        while (NULL != (de = readdir(dp))) {
            ...
            if (!found) {
                ww_argv_append_nosize(&good_files, abs_name);
            }
            free(abs_name);
        }
        closedir(dp);
 ...
 * Module definition
 */
ww_dl_base_module_t ww_dl_dlopen_module = {
    .open = dlopen_open,
