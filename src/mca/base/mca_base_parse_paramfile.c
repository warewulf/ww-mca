...
static ww_list_t * _param_list;

int mca_base_parse_paramfile(const char *paramfile, ww_list_t *list)
{
    file_being_read = (char*)paramfile;
    _param_list = list;

    return ww_util_keyval_parse(paramfile, save_value);
}

int mca_base_internal_env_store(void)
{
    return ww_util_keyval_save_internal_envars(save_value);
}
...
static void save_value(const char *name, const char *value)
{
    ...
    WW_LIST_FOREACH(fv, _param_list, mca_base_var_file_value_t) {
    ...
    if (!found) {
        ...
        fv = WW_NEW(mca_base_var_file_value_t);
        if (NULL == fv) {
            return;
        }

        fv->mbvfv_var = strdup(name);
        ww_list_append(_param_list, &fv->super);
    }

    fv->mbvfv_value = value ? strdup(value) : NULL;
    fv->mbvfv_file  = file_being_read;
    fv->mbvfv_lineno = ww_util_keyval_parse_lineno;
}
