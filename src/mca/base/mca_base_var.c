 ...
 * local variables
 */
static ww_pointer_array_t mca_base_vars;
static const char *mca_prefix = "WW_MCA_";
...
static ww_list_t mca_base_var_file_values;
static ww_list_t mca_base_envar_file_values;
static ww_list_t mca_base_var_override_values;

static int mca_base_var_count = 0;

static ww_hash_table_t mca_base_var_index_hash;
...
WW_CLASS_INSTANCE(mca_base_var_t, ww_object_t,
...
WW_CLASS_INSTANCE(mca_base_var_file_value_t, ww_list_item_t,
...
static char *append_filename_to_list(const char *filename)
{
    ...
    (void) ww_argv_append_unique_nosize(&mca_base_var_file_list, filename, false);

    count = ww_argv_count(mca_base_var_file_list);
...
int mca_base_var_init(void)
{
    ...
    if (!mca_base_var_initialized) {
        ...
        WW_CONSTRUCT(&mca_base_vars, ww_pointer_array_t);
        /* These values are arbitrary */
        ret = ww_pointer_array_init (&mca_base_vars, 128, 16384, 128);
        if (WW_SUCCESS != ret) {
            return ret;
        }
        ...
        WW_CONSTRUCT(&mca_base_var_file_values, ww_list_t);
        WW_CONSTRUCT(&mca_base_envar_file_values, ww_list_t);
        WW_CONSTRUCT(&mca_base_var_override_values, ww_list_t);
        WW_CONSTRUCT(&mca_base_var_index_hash, ww_hash_table_t);

        ret = ww_hash_table_init (&mca_base_var_index_hash, 1024);
        if (WW_SUCCESS != ret) {
            return ret;
        }
        ...
        (void)mca_base_var_register ("Warewulf", "mca", "base", "env_list",
                                     "Set SHELL env variables",
                                     MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, WW_INFO_LVL_3,
                                     MCA_BASE_VAR_SCOPE_READONLY, &mca_base_env_list);

        mca_base_env_list_sep = MCA_BASE_ENV_LIST_SEP_DEFAULT;
        (void)mca_base_var_register ("Warewulf", "mca", "base", "env_list_delimiter",
        ...
        if (NULL != mca_base_env_list) {
            ...
            if (NULL != name) {
                ww_setenv(name, mca_base_env_list, false, &environ);
                free(name);
            }
        }
         ...
         */
        (void)mca_base_var_register ("Warewulf", "mca", "base", "env_list_internal",
                "Store SHELL env variables from amca conf file",
                MCA_BASE_VAR_TYPE_STRING, NULL, 0, MCA_BASE_VAR_FLAG_INTERNAL, WW_INFO_LVL_3,
...
static void process_env_list(char *env_list, char ***argv, char sep)
{
    ...
    tokens = ww_argv_split(env_list, (int)sep);
    if (NULL == tokens) {
        return;
    }
    ...
    for (int i = 0 ; NULL != tokens[i] ; ++i) {
        if (NULL == (ptr = strchr(tokens[i], '='))) {
            ...
            if (NULL == value) {
                ww_show_help("help-mca-var.txt", "incorrect-env-list-param",
            ...
            if (NULL != (ptr = strchr(value, '='))) {
                *ptr = '\0';
                ww_setenv(value, ptr + 1, true, argv);
            } else {
                ww_setenv(tokens[i], value, true, argv);
            }

            free (value);
        } else {
            *ptr = '\0';
            ww_setenv(tokens[i], ptr + 1, true, argv);
...
int mca_base_var_process_env_list(char ***argv)
{
    ...
    if (NULL != mca_base_env_list_sep) {
        if (1 == strlen(mca_base_env_list_sep)) {
            ...
            ww_show_help("help-mca-var.txt", "incorrect-env-list-sep",
...
static void resolve_relative_paths(char **file_prefix, char *file_path, bool rel_path_search, char **files, char sep)
{
     ...
     */
    if( WW_SUCCESS != fixup_files(file_prefix, file_path, rel_path_search, sep) ) {
#if 0
        /* JJH We need to die! */
        abort();
#else
...
static int mca_base_var_cache_files(bool rel_path_search)
{
    ...
    home = (char*)ww_home_directory();

    if(NULL == cwd) {
        cwd = (char *) malloc(sizeof(char) * MAXPATHLEN);
        if( NULL == (cwd = getcwd(cwd, MAXPATHLEN) )) {
            ww_output(0, "Error: Unable to get the current working directory\n");
            cwd = strdup(".");
        }
    }

#if WW_WANT_HOME_CONFIG_FILES
    asprintf(&mca_base_var_files, "%s"WW_PATH_SEP".openmpi" WW_PATH_SEP
             "mca-params.conf%c%s" WW_PATH_SEP "openmpi-mca-params.conf",
             home, WW_ENV_SEP, ww_install_dirs.sysconfdir);
#else
    asprintf(&mca_base_var_files, "%s" WW_PATH_SEP "openmpi-mca-params.conf",
    ...
    ret = mca_base_var_register ("Warewulf", "mca", "base", "param_files", "Path for MCA "
                                 "configuration files containing variable values",
                                 MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, WW_INFO_LVL_2,
    ...
    (void) mca_base_var_register_synonym (ret, "Warewulf", "mca", NULL, "param_files",
                                          MCA_BASE_VAR_SYN_FLAG_DEPRECATED);

    ret = asprintf(&mca_base_var_override_file, "%s" WW_PATH_SEP "openmpi-mca-params-override.conf",
                   ww_install_dirs.sysconfdir);
    if (0 > ret) {
        return WW_ERR_OUT_OF_RESOURCE;
    }
    ...
    ret = mca_base_var_register ("Warewulf", "mca", "base", "override_param_file",
                                 ...
                                 WW_INFO_LVL_2, MCA_BASE_VAR_SCOPE_CONSTANT,
    ...
    ret = mca_base_var_register ("Warewulf", "mca", "base", "suppress_override_warning",
                                 "Suppress warnings when attempting to set an overridden value (default: false)",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, WW_INFO_LVL_2,
    ...
    ret = mca_base_var_register ("Warewulf", "mca", "base", "param_file_prefix",
    ...
    ret = mca_base_var_register ("Warewulf", "mca", "base", "envar_file_prefix",
    ...
    ret = mca_base_var_register ("Warewulf", "mca", "base", "param_file_path",
    ...
    force_agg_path = NULL;
    ret = mca_base_var_register ("Warewulf", "mca", "base", "param_file_path_force",
    ...
    if (NULL != force_agg_path) {
        if (NULL != mca_base_param_file_path) {
            ...
            asprintf(&mca_base_param_file_path, "%s%c%s", force_agg_path, WW_ENV_SEP, tmp_str);
            free(tmp_str);
        } else {
    ...
    if (NULL != mca_base_var_file_prefix) {
       resolve_relative_paths(&mca_base_var_file_prefix, mca_base_param_file_path, rel_path_search, &mca_base_var_files, WW_ENV_SEP);
    }
    read_files (mca_base_var_files, &mca_base_var_file_values, WW_ENV_SEP);
    ...
    if (0 == access(mca_base_var_override_file, F_OK)) {
        read_files (mca_base_var_override_file, &mca_base_var_override_values, WW_ENV_SEP);
    }
...
static int var_set_from_string (mca_base_var_t *var, char *src)
{
    ...
    case MCA_BASE_VAR_TYPE_SIZE_T:
        ...
        if (WW_ERR_VALUE_OUT_OF_BOUNDS == ret ||
            ...
            if (var->mbv_enumerator) {
                ...
                ww_show_help("help-mca-var.txt", "invalid-value-enum",
                               true, var->mbv_full_name, src, valid_values);
                free(valid_values);
            } else {
                ww_show_help("help-mca-var.txt", "invalid-value",
...
static int var_get (int vari, mca_base_var_t **var_out, bool original)
{
    ...
    var = ww_pointer_array_get_item (&mca_base_vars, vari);
    if (NULL == var) {
        return WW_ERR_BAD_PARAM;
    }
...
static int var_find_by_name (const char *full_name, int *vari, bool invalidok)
{
    ...
    rc = ww_hash_table_get_value_ptr (&mca_base_var_index_hash, full_name, strlen (full_name),
                                        &tmp);
    if (WW_SUCCESS != rc) {
        return rc;
    }
...
int mca_base_var_set_flag (int vari, mca_base_var_flag_t flag, bool set)
{
    ...
    ret = var_get (vari, &var, true);
    if (WW_SUCCESS != ret || VAR_IS_SYNONYM(var[0])) {
        return WW_ERR_BAD_PARAM;
    }
...
int mca_base_var_build_env(char ***env, int *num_env, bool internal)
{
    ...
    len = ww_pointer_array_get_size(&mca_base_vars);
    for (i = 0; i < len; ++i) {
        char *value_string;
        char *str = NULL;

        var = ww_pointer_array_get_item (&mca_base_vars, i);
        if (NULL == var) {
            continue;
        }
        ...
        ww_argv_append(num_env, env, str);
        free(str);
...
{
    ww_object_t *Warewulfect;
    ww_list_item_t *item;
    int size, i;

    if (mca_base_var_initialized) {
        size = ww_pointer_array_get_size(&mca_base_vars);
        for (i = 0 ; i < size ; ++i) {
            Warewulfect = ww_pointer_array_get_item (&mca_base_vars, i);
            if (NULL != Warewulfect) {
                WW_RELEASE(Warewulfect);
            }
        }
        WW_DESTRUCT(&mca_base_vars);

        while (NULL !=
               (item = ww_list_remove_first(&mca_base_var_file_values))) {
            WW_RELEASE(item);
        }
        WW_DESTRUCT(&mca_base_var_file_values);

        while (NULL !=
               (item = ww_list_remove_first(&mca_base_envar_file_values))) {
            WW_RELEASE(item);
        }
        WW_DESTRUCT(&mca_base_envar_file_values);

        while (NULL !=
               (item = ww_list_remove_first(&mca_base_var_override_values))) {
            WW_RELEASE(item);
        }
        WW_DESTRUCT(&mca_base_var_override_values);
        ...
        if (NULL != mca_base_var_file_list) {
            ww_argv_free(mca_base_var_file_list);
        }
        mca_base_var_file_list = NULL;

        (void) mca_base_var_group_finalize ();

        WW_DESTRUCT(&mca_base_var_index_hash);
...
static int fixup_files(char **file_list, char * path, bool rel_path_search, char sep) {
    ...
    search_path = ww_argv_split(path, WW_ENV_SEP);
    files = ww_argv_split(*file_list, sep);
    count = ww_argv_count(files);
    ...
    for (i = 0 ; i < count; ++i) {
        char *msg_path = path;
        if (ww_path_is_absolute(files[i])) {
            /* Absolute paths preserved */
            tmp_file = ww_path_access(files[i], NULL, mode);
        } else if (!rel_path_search && NULL != strchr(files[i], WW_PATH_SEP[0])) {
            /* Resolve all relative paths:
             ...
             */
            msg_path = rel_path;
            tmp_file = ww_path_access(files[i], rel_path, mode);
        } else {
            /* Resolve all relative paths:
             * - Use path resolution
             ...
             */
            tmp_file = ww_path_find (files[i], search_path, mode, NULL);
        }

        if (NULL == tmp_file) {
            ww_show_help("help-mca-var.txt", "missing-param-file",
                           true, getpid(), files[i], msg_path);
            exit_status = WW_ERROR;
            break;
        }

        ww_argv_append(&argc, &argv, tmp_file);

        free(tmp_file);
        tmp_file = NULL;
    }

    if (WW_SUCCESS == exit_status) {
        free(*file_list);
        *file_list = ww_argv_join(argv, sep);
    }
...
static int read_files(char *file_list, ww_list_t *file_values, char sep)
{
    char **tmp = ww_argv_split(file_list, sep);
    int i, count;
                              ...
                              void *storage)
{
    ...
    switch (type) {
    case MCA_BASE_VAR_TYPE_INT:
        align = WW_ALIGNMENT_INT;
        break;
    case MCA_BASE_VAR_TYPE_UNSIGNED_INT:
        align = WW_ALIGNMENT_INT;
        break;
    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG:
        align = WW_ALIGNMENT_LONG;
        break;
    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG_LONG:
        align = WW_ALIGNMENT_LONG_LONG;
        break;
    case MCA_BASE_VAR_TYPE_SIZE_T:
        align = WW_ALIGNMENT_SIZE_T;
        break;
    case MCA_BASE_VAR_TYPE_BOOL:
        align = WW_ALIGNMENT_BOOL;
        break;
    case MCA_BASE_VAR_TYPE_DOUBLE:
        align = WW_ALIGNMENT_DOUBLE;
        break;
    case MCA_BASE_VAR_TYPE_VERSION_STRING:
    case MCA_BASE_VAR_TYPE_STRING:
    default:
    ...
    if (flags & MCA_BASE_VAR_FLAG_SYNONYM) {
        original = ww_pointer_array_get_item (&mca_base_vars, synonym_for);
        if (NULL == original) {
    ...
    if (0 > var_index) {
        ...
        if (scope < MCA_BASE_VAR_SCOPE_LOCAL || (flags & MCA_BASE_VAR_FLAG_DEFAULT_ONLY)) {
            if ((flags & MCA_BASE_VAR_FLAG_DEFAULT_ONLY) && (flags & MCA_BASE_VAR_FLAG_SETTABLE)) {
                ww_show_help("help-mca-var.txt", "invalid-flag-combination",
        ...
        var_index = ww_pointer_array_add (&mca_base_vars, var);
        if (0 > var_index) {
            WW_RELEASE(var);
            return WW_ERROR;
        }
        ...
        ww_hash_table_set_value_ptr (&mca_base_var_index_hash, var->mbv_full_name, strlen (var->mbv_full_name),
        ...
        if (0 != compare_strings(framework_name, group->group_framework) ||
            ...
            ww_show_help("help-mca-var.txt", "var-name-conflict",
    ...
    if (!original) {
        ...
        /* synonym variable */
        ww_value_array_append_item(&original->mbv_synonyms, &var_index);
    }
...
static int var_set_from_env (mca_base_var_t *var, mca_base_var_t *original)
{
    ...
    if (VAR_IS_DEFAULT_ONLY(original[0])) {
        ww_show_help("help-mca-var.txt", "default-only-param-set",
                       true, var_full_name);
    ...
    if (MCA_BASE_VAR_SOURCE_OVERRIDE == original->mbv_source) {
        if (!mca_base_var_suppress_override_warning) {
            ww_show_help("help-mca-var.txt", "overridden-param-set",
                           true, var_full_name);
        }
    ...
    if (deprecated) {
        ...
        switch (var->mbv_source) {
        case MCA_BASE_VAR_SOURCE_ENV:
            ww_show_help("help-mca-var.txt", "deprecated-mca-env",
                           true, var_full_name, new_variable);
            break;
        case MCA_BASE_VAR_SOURCE_COMMAND_LINE:
            ww_show_help("help-mca-var.txt", "deprecated-mca-cli",
                           true, var_full_name, new_variable);
            break;
        case MCA_BASE_VAR_SOURCE_FILE:
        case MCA_BASE_VAR_SOURCE_OVERRIDE:
            ww_show_help("help-mca-var.txt", "deprecated-mca-file",
 ...
 */
static int var_set_from_file (mca_base_var_t *var, mca_base_var_t *original, ww_list_t *file_values)
{
    const char *var_full_name = var->mbv_full_name;
    const char *var_long_name = var->mbv_long_name;
    bool deprecated = VAR_IS_DEPRECATED(var[0]);
    ...
    WW_LIST_FOREACH(fv, file_values, mca_base_var_file_value_t) {
        ...
        if (MCA_BASE_VAR_FLAG_ENVIRONMENT_ONLY & original->mbv_flags) {
            ww_show_help("help-mca-var.txt", "environment-only-param",
...
static void var_constructor(mca_base_var_t *var)
{
    ...
    WW_CONSTRUCT(&var->mbv_synonyms, ww_value_array_t);
    ww_value_array_init (&var->mbv_synonyms, sizeof (int));
}
...
static int var_value_string (mca_base_var_t *var, char **value_string)
{
    ...
    if (NULL == var->mbv_enumerator) {
        ...
        ret = (0 > ret) ? WW_ERR_OUT_OF_RESOURCE : WW_SUCCESS;
    } else {
...
int mca_base_var_dump(int vari, char ***out, mca_base_var_dump_type_t output_type)
{
    ...
    synonym_count = ww_value_array_get_size(&var->mbv_synonyms);
    if (synonym_count) {
        synonyms = WW_VALUE_ARRAY_GET_BASE(&var->mbv_synonyms, int);
    }
    ...
    if (MCA_BASE_VAR_DUMP_PARSABLE == output_type) {
        ...
        *out = (char **) calloc (line_count + 1, sizeof (char *));
        ...
        } else if (ww_value_array_get_size(&var->mbv_synonyms)) {
