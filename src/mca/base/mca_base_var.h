...
#ifndef WW_MCA_BASE_VAR_H
#define WW_MCA_BASE_VAR_H
...
#include "src/mca/base/mca_base_var_group.h"
...
struct mca_base_var_t {
    ...
    /** List of synonym names for this variable.  This *must* be a
        pointer (vs. a plain ww_list_t) because we copy this whole
        struct into a new var for permanent storage
        (ww_vale_array_append_item()), and the internal pointers in
        the ww_list_t will be invalid when that happens.  Hence, we
        simply keep a pointer to an external ww_list_t.  Synonyms
        are uncommon enough that this is not a big performance hit. */
    ww_value_array_t mbv_synonyms;
 ...
 */
WW_DECLSPEC WW_CLASS_DECLARATION(mca_base_var_t);
 ...
 */
WW_DECLSPEC int mca_base_var_init(void);
 ...
 * @retval WW_ERR_OUT_OF_RESOURCE Upon failure to allocate memory.
 * @retval WW_ERROR Upon failure to register the variable.
 *
 ...
 * successfully.
 */
WW_DECLSPEC int mca_base_var_register (const char *project_name, const char *framework_name,
 ...
 * its framework.
 */
WW_DECLSPEC int mca_base_component_var_register (const mca_base_component_t *component,
 ...
 */
WW_DECLSPEC int mca_base_framework_var_register (const mca_base_framework_t *framework,
 ...
 * @returns WW_ERR_BAD_VAR If synonym_for does not reference a valid
 * variable.
 * @returns WW_ERR_OUT_OF_RESOURCE If memory could not be allocated.
 * @returns WW_ERROR For all other errors.
 *
 ...
 */
WW_DECLSPEC int mca_base_var_register_synonym (int synonym_for, const char *project_name,
 ...
 */
WW_DECLSPEC int mca_base_var_deregister(int vari);
 ...
 *
 * @return WW_ERROR Upon failure.  The contents of value are
 * undefined.
 * @return WW_SUCCESS Upon success. value (if not NULL) will be filled
 ...
 */
WW_DECLSPEC int mca_base_var_get_value (int vari, const void *value,
 ...
 * @retval WW_ERR_PERM If the variable is not settable.
 * @retval WW_ERR_BAD_PARAM If the variable does not exist or has
 * been deregistered.
 * @retval WW_ERROR On other error.
 *
 ...
 * settable.
 */
WW_DECLSPEC int mca_base_var_set_value (int vari, const void *value, size_t size,
                                          mca_base_var_source_t source,
                                          const char *source_file);
 ...
 */
WW_DECLSPEC int mca_base_var_env_name(const char *param_name,
                                        char **env_name);
 ...
 *
 * @retval WW_ERROR If the variable was not found.
 ...
 */
WW_DECLSPEC int mca_base_var_find (const char *project_name,
 ...
 */
WW_DECLSPEC int mca_base_var_find_by_name (const char *full_name, int *vari);
 ...
 *
 * This function will print an ww_show_help() message and return
 * WW_ERR_BAD_VAR if it finds that the two variables both have
 ...
 * not default values).
 *
 * Note that ww_show_help() allows itself to be hooked, so if this
 * happens after the aggregated ww_show_help() system is
 * initialized, the messages will be aggregated (w00t).
 *
 * @returns WW_ERR_BAD_VAR if the two variables have sources that
 * are not MCA_BASE_VAR_SOURCE_DEFAULT.
 * @returns WW_SUCCESS otherwise.
 */
WW_DECLSPEC int mca_base_var_check_exclusive (const char *project,
                                                const char *type_a,
 ...
 *
 * @returns WW_SUCCESS If the flags are set successfully.
 * @returns WW_ERR_BAD_PARAM If the variable is not registered.
 * @returns WW_ERROR Otherwise
 */
WW_DECLSPEC int mca_base_var_set_flag(int vari, mca_base_var_flag_t flag,
                                        bool set);
 ...
 */
WW_DECLSPEC int mca_base_var_get_count (void);
 ...
 */
WW_DECLSPEC int mca_base_var_build_env(char ***env, int *num_env,
                                         bool internal);
 ...
 *
 * @returns WW_SUCCESS This function never fails.
 *
 ...
 */
WW_DECLSPEC int mca_base_var_finalize(void);
 ...
 */
WW_DECLSPEC int mca_base_var_dump(int vari, char ***out, mca_base_var_dump_type_t output_type);
