...
#ifndef WW_VALUE_ARRAY_H
#define WW_VALUE_ARRAY_H
...
typedef struct ww_value_array_t ww_value_array_t;

WW_DECLSPEC WW_CLASS_DECLARATION(ww_value_array_t);
...
static inline int ww_value_array_init(ww_value_array_t *array, size_t item_sizeof)
{
    ...
    return (NULL != array->array_items) ? WW_SUCCESS : WW_ERR_OUT_OF_RESOURCE;
}
...
static inline int ww_value_array_reserve(ww_value_array_t* array, size_t size)
{
...
static inline size_t ww_value_array_get_size(ww_value_array_t* array)
{
 ...
 *  case, if the routine is successful, ww_value_array_get_size() will
 *  return the new size.
 */

WW_DECLSPEC int ww_value_array_set_size(ww_value_array_t* array, size_t size);
...
#define WW_VALUE_ARRAY_GET_ITEM(array, item_type, item_index) \
...
static inline void* ww_value_array_get_item(ww_value_array_t *array, size_t item_index)
{
    if(item_index >= array->array_size && ww_value_array_set_size(array, item_index+1) != WW_SUCCESS)
        return NULL;
...
#define WW_VALUE_ARRAY_SET_ITEM(array, item_type, item_index, item_value) \
...
static inline int ww_value_array_set_item(ww_value_array_t *array, size_t item_index, const void* item)
{
    int rc;
    if(item_index >= array->array_size &&
       (rc = ww_value_array_set_size(array, item_index+1)) != WW_SUCCESS)
        return rc;
...
static inline int ww_value_array_append_item(ww_value_array_t *array, const void *item)
{
    return ww_value_array_set_item(array, array->array_size, item);
}
...
static inline int ww_value_array_remove_item(ww_value_array_t *array, size_t item_index)
{
#if WW_ENABLE_DEBUG
    if (item_index >= array->array_size) {
        ww_output(0, "ww_value_array_remove_item: invalid index %lu\n", (unsigned long)item_index);
 ...
 * than looping over WW_VALUE_ARRAY_GET_ITEM() and
 * WW_VALUE_ARRAY_SET_ITEM() because it will [potentially] reduce the
 * number of pointer dereferences.
 */

#define WW_VALUE_ARRAY_GET_BASE(array, item_type) \
