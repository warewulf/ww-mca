 ...
 *   given time (i.e., when an ww timer event expires).
 ...
 *
 * There is an ww_hotel_init() function to create a hotel, but no
...
typedef void (*ww_hotel_eviction_callback_fn_t)(struct ww_hotel_t *hotel,
                                                  int room_num,
                                                  void *occupant);

/* Note that this is an internal data structure; it is not part of the
   public ww_hotel interface.  Public consumers of ww_hotel
...
typedef struct {
    void *occupant;
    ww_event_t eviction_timer_event;
} ww_hotel_room_t;
...
WW_CLASS_DECLARATION(ww_hotel_t);
 ...
 *
 * @return WW_SUCCESS if all initializations were succesful. Otherwise,
 *  the error indicate what went wrong in the function.
 */
WW_DECLSPEC int ww_hotel_init(ww_hotel_t *hotel, int num_rooms,
                                  ww_event_base_t *evbase,
 ...
 *
 * @return WW_SUCCESS if the occupant is successfully checked in,
 * and the room parameter will contain a valid value.
 * @return WW_ERR_TEMP_OUT_OF_RESOURCE is the hotel is full.  Try
 * again later.
 */
static inline int ww_hotel_checkin(ww_hotel_t *hotel,
                                     void *occupant,
                                     int *room_num)
{
    ww_hotel_room_t *room;

    /* Do we have any rooms available? */
    if (WW_UNLIKELY(hotel->last_unoccupied_room < 0)) {
...
/**
 * Same as ww_hotel_checkin(), but slightly optimized for when the
 * caller *knows* that there is a room available.
 */
static inline void ww_hotel_checkin_with_res(ww_hotel_t *hotel,
                                     void *occupant,
                                     int *room_num)
{
    ww_hotel_room_t *room;
 ...
 */
static inline void ww_hotel_checkout(ww_hotel_t *hotel, int room_num)
{
    ww_hotel_room_t *room;
    ...
    room = &(hotel->rooms[room_num]);
    if (WW_LIKELY(NULL != room->occupant)) {
        /* Do not change this logic without also changing the same
           logic in ww_hotel_checkout_and_return_occupant() and
           ww_hotel.c:local_eviction_callback(). */
        room->occupant = NULL;
        if (NULL != hotel->evbase) {
 ...
 */
static inline void ww_hotel_checkout_and_return_occupant(ww_hotel_t *hotel, int room_num, void **occupant)
{
    ww_hotel_room_t *room;
    ...
    room = &(hotel->rooms[room_num]);
    if (WW_LIKELY(NULL != room->occupant)) {
        ww_output (10, "checking out occupant %p from room num %d", room->occupant, room_num);
        /* Do not change this logic without also changing the same
           logic in ww_hotel_checkout() and
           ww_hotel.c:local_eviction_callback(). */
 ...
 *
 */
static inline bool ww_hotel_is_empty (ww_hotel_t *hotel)
{
 ...
 */
static inline void ww_hotel_knock(ww_hotel_t *hotel, int room_num, void **occupant)
{
    ww_hotel_room_t *room;
    ...
    *occupant = NULL;
    ...
    room = &(hotel->rooms[room_num]);
    if (WW_LIKELY(NULL != room->occupant)) {
        ww_output (10, "occupant %p in room num %d responded to knock", room->occupant, room_num);
        *occupant = room->occupant;
    }
}
