...
static void ww_pointer_array_construct(ww_pointer_array_t *);
static void ww_pointer_array_destruct(ww_pointer_array_t *);
static bool grow_table(ww_pointer_array_t *table, int soft, int hard);

WW_CLASS_INSTANCE(ww_pointer_array_t, ww_object_t,
                   ww_pointer_array_construct,
                   ww_pointer_array_destruct);

/*
 * ww_pointer_array constructor
 */
static void ww_pointer_array_construct(ww_pointer_array_t *array)
{
    array->lowest_free = 0;
    array->number_free = 0;
    array->size = 0;
...
/*
 * ww_pointer_array destructor
 */
static void ww_pointer_array_destruct(ww_pointer_array_t *array)
{
    /* free table */
    if( NULL != array->addr) {
        free(array->addr);
 ...
 */
ww_status_t ww_pointer_array_init(ww_pointer_array_t* array,
 ...
 *
 * @param table Pointer to ww_pointer_array_t object (IN)
 * @param ptr Pointer to be added to table    (IN)
 *
 * @return Array index where ptr is inserted or WW_ERROR if it fails
 */
int ww_pointer_array_add(ww_pointer_array_t *table, void *ptr)
{
    int i, index;
 ...
 */
ww_status_t ww_pointer_array_set_item(ww_pointer_array_t *table, int index,
                                          void * value)
{
    assert(table != NULL);
...
#if 0
    ww_output(0,"ww_pointer_array_set_item: OUT: "
 ...
 */
bool ww_pointer_array_test_and_set_item (ww_pointer_array_t *table,
...
#if 0
    ww_output(0,"ww_pointer_array_test_and_set_item: IN:  "
               " table %p (size %ld, lowest free %ld, number free %ld)"
               " addr[%d] = %p\n",
               table, table->size, table->lowest_free, table->number_free,
               index, table->addr[index]);
...
#if 0
    ww_output(0,"ww_pointer_array_test_and_set_item: OUT: "
               " table %p (size %ld, lowest free %ld, number free %ld)"
               " addr[%d] = %p\n",
               table, table->size, table->lowest_free, table->number_free,
               index, table->addr[index]);
...
ww_status_t ww_pointer_array_set_size(ww_pointer_array_t *array, int new_size)
{
