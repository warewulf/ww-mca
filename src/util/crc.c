...
#if (WW_ALIGNMENT_LONG == 8)
#define WW_CRC_WORD_MASK_ 0x7
#elif (WW_ALIGNMENT_LONG == 4)
#define WW_CRC_WORD_MASK_ 0x3
#else
#define WW_CRC_WORD_MASK_ 0xFFFF
#endif


#define WORDALIGNED(v) \
    (((intptr_t)v & WW_CRC_WORD_MASK_) ? false : true)
...
static bool _ww_crc_table_initialized = false;
static unsigned int _ww_crc_table[256];
...
void ww_initialize_crc_table(void)
{
    ...
    for (i = 0; i < 256; i++) {
        ...
        _ww_crc_table[i] = crc_accum;
    }

    /* set global bool to true to do this work once! */
    _ww_crc_table_initialized = true;
    return;
}

unsigned int ww_bcopy_uicrc_partial(
    const void *  source,
    void *  destination,
    size_t copylen,
    size_t crclen,
    unsigned int partial_crc)
{
    ...
    if (!_ww_crc_table_initialized) {
        ww_initialize_crc_table();
    }
        ...
        while (copylen >= sizeof(unsigned int)) {
            ...
            *dst++ = tmp;
            ...
            for (j = 0; j < (int)sizeof(unsigned int); j++) {
                ...
                partial_crc = (partial_crc << 8) ^ _ww_crc_table[i];
            }
