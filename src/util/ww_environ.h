 ...
 * later be freed with a call to ww_argv_free().
 *
 ...
 */
WW_DECLSPEC char **ww_environ_merge(char **minor, char **major) __ww_attribute_warn_unused_result__;
 ...
 *
 * @retval WW_ERR_OUT_OF_RESOURCE If internal malloc() fails.
 * @retval WW_EXISTS If the name already exists in \em env and
 * \em overwrite is false (and therefore the \em value was not
 * saved in \em env)
 * @retval WW_SUCESS If the value replaced another value or is
 * appended to \em env.
 *
 ...
 *
 * \code
 *   #include "ww/util/ww_environ.h"
 *   ww_setenv("foo", "bar", true, &environ);
 * \endcode
 *
 * NOTE: If you use the real environ, ww_setenv() will turn
 ...
 * \code
 *   char **my_env = NULL;
 *   ww_setenv("foo", "bar", true, &my_env);
 * \endcode
 */
WW_DECLSPEC int ww_setenv(const char *name, const char *value,
                              bool overwrite, char ***env) __ww_attribute_nonnull__(1);
 ...
 * @param env The environment to use
 *
 * @retval WW_ERR_OUT_OF_RESOURCE If an internal malloc fails.
 * @retval WW_ERR_NOT_FOUND If \em name is not found in \em env.
 * @retval WW_SUCCESS If \em name is found and successfully deleted.
 *
 * If \em name is found in \em env, the string corresponding to
 * that entry is freed and its entry is eliminated from the array.
 */
WW_DECLSPEC ww_status_t ww_unsetenv(const char *name, char ***env) __ww_attribute_nonnull__(1);

/* A consistent way to retrieve the home and tmp directory on all supported
 * platforms.
 */
WW_DECLSPEC const char* ww_home_directory( void );
WW_DECLSPEC const char* ww_tmp_directory( void );
...
#else
WW_DECLSPEC extern char **environ;
#endif
