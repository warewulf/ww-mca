...
static ww_output_stream_t verbose;
 ...
 * Private functions
 */
static void construct(ww_object_t *stream);
...
#define WW_OUTPUT_MAX_STREAMS 64
...
/* global state */
bool ww_output_redirected_to_syslog = false;
int ww_output_redirected_syslog_pri = 0;
...
static output_desc_t info[WW_OUTPUT_MAX_STREAMS];
...
WW_CLASS_INSTANCE(ww_output_stream_t, ww_object_t, construct, NULL);

/*
 * Setup the output stream infrastructure
 */
bool ww_output_init(void)
{
    int i;
    char hostname[32];
    char *str;
    ...
    str = getenv("WW_OUTPUT_STDERR_FD");
    if (NULL != str) {
    ...
    if (NULL != str) {
        if (0 == strcasecmp(str, "syslog")) {
            ww_output_redirected_to_syslog = true;
        }
    }
    str = getenv("WW_OUTPUT_SYSLOG_PRI");
    if (NULL != str) {
        if (0 == strcasecmp(str, "info")) {
            ww_output_redirected_syslog_pri = LOG_INFO;
        } else if (0 == strcasecmp(str, "error")) {
            ww_output_redirected_syslog_pri = LOG_ERR;
        } else if (0 == strcasecmp(str, "warn")) {
            ww_output_redirected_syslog_pri = LOG_WARNING;
        } else {
            ww_output_redirected_syslog_pri = LOG_ERR;
        }
    } else {
    ...
    str = getenv("WW_OUTPUT_SYSLOG_IDENT");
    if (NULL != str) {
        redirect_syslog_ident = strdup(str);
    }

    WW_CONSTRUCT(&verbose, ww_output_stream_t);
    if (ww_output_redirected_to_syslog) {
        verbose.lds_want_syslog = true;
        verbose.lds_syslog_priority = ww_output_redirected_syslog_pri;
        if (NULL != str) {
    ...
    for (i = 0; i < WW_OUTPUT_MAX_STREAMS; ++i) {
        ...
        info[i].ldi_syslog = ww_output_redirected_to_syslog;
    ...
    output_dir = strdup(ww_tmp_directory());

    /* Open the default verbose stream */
    verbose_stream = ww_output_open(&verbose);
    return true;
}
 ...
 * Open a stream
 */
int ww_output_open(ww_output_stream_t * lds)
{
 ...
 */
int ww_output_reopen(int output_id, ww_output_stream_t * lds)
{
 ...
 */
bool ww_output_switch(int output_id, bool enable)
{
    bool ret = false;
 ...
 */
void ww_output_reopen_all(void)
{
    char *str;
    char hostname[32];
    ...
    for (i = 0; i < WW_OUTPUT_MAX_STREAMS; ++i) {
        ...
        /*
         * set this to zero to ensure that ww_output_open will
        ...
        /*
         * call ww_output_open to open the stream. The return value
 ...
 */
void ww_output(int output_id, const char *format, ...)
{
    if (output_id >= 0 && output_id < WW_OUTPUT_MAX_STREAMS) {
        va_list arglist;
 ...
 */
void ww_output_verbose(int level, int output_id, const char *format, ...)
{
    if (output_id >= 0 && output_id < WW_OUTPUT_MAX_STREAMS &&
 ...
 * Send a message to a stream if the verbose level is high enough
 */
void ww_output_vverbose(int level, int output_id, const char *format,
                          va_list arglist)
{
    if (output_id >= 0 && output_id < WW_OUTPUT_MAX_STREAMS &&
        info[output_id].ldi_verbose_level >= level) {
 ...
 */
char *ww_output_string(int level, int output_id, const char *format, ...)
{
    int rc;
    char *ret = NULL;
 ...
 * Send a message to a string if the verbose level is high enough
 */
char *ww_output_vstring(int level, int output_id, const char *format,
                          va_list arglist)
{
    int rc;
    char *ret = NULL;
 ...
 */
void ww_output_set_verbosity(int output_id, int level)
{
    if (output_id >= 0 && output_id < WW_OUTPUT_MAX_STREAMS) {
        info[output_id].ldi_verbose_level = level;
    }
 ...
 */
void ww_output_set_output_file_info(const char *dir,
                                      const char *prefix,
                                      char **olddir,
                                      char **oldprefix)
{
...
void ww_output_hexdump(int verbose_level, int output_id,
                         void *ptr, int buflen)
{
    ...
    if (output_id >= 0 && output_id < WW_OUTPUT_MAX_STREAMS &&
        ...
        ww_output_verbose(verbose_level, output_id, "dump data at %p %d bytes\n", ptr, buflen);
        for (i = 0; i < buflen; i += 16) {
            ...
            if (ret < 0)
            return;
            ww_output_verbose(verbose_level, output_id, "%s", out_buf);
        }
    }
}
 ...
 */
void ww_output_finalize(void)
{
    if (initialized) {
        if (verbose_stream != -1) {
            ww_output_close(verbose_stream);
        }
        free(verbose.lds_prefix);
        verbose_stream = -1;
 ...
 * Constructor
 */
static void construct(ww_object_t *obj)
{
    ww_output_stream_t *stream = (ww_output_stream_t*) obj;
...
static int open_file(int i)
{
     ...
     * to open it twice
     */
    for (n=0; n < WW_OUTPUT_MAX_STREAMS; n++) {
        if (i == n) {
            continue;
        }
        if (!info[n].ldi_used) {
    ...
    if (NULL != output_dir) {
        filename = (char *) malloc(WW_PATH_MAX);
        if (NULL == filename) {
            return WW_ERR_OUT_OF_RESOURCE;
        }
        strncpy(filename, output_dir, WW_PATH_MAX);
        strcat(filename, "/");
...
static int output(int output_id, const char *format, va_list arglist)
{
    ...
    if (output_id >= 0 && output_id < WW_OUTPUT_MAX_STREAMS &&
        ...
        /* Make the strings */
        if (WW_SUCCESS != (rc = make_string(&str, ldi, format, arglist))) {
            return rc;
        }
         ...
         * ww_output.h docs about this!). */

        if (ldi->ldi_file) {
            if (ldi->ldi_fd == -1) {
                if (WW_SUCCESS != open_file(output_id)) {
                    ...
                    snprintf(buffer, BUFSIZ - 1,
                             "[WARNING: %d lines lost because the Warewulf process session directory did\n not exist when ww_output() was invoked]\n",
                             ldi->ldi_file_num_lines_lost);
...
int ww_output_get_verbosity(int output_id)
{
    if (output_id >= 0 && output_id < WW_OUTPUT_MAX_STREAMS && info[output_id].ldi_used) {
