 ...
 * Local functions
 */
static int ww_show_vhelp_internal(const char *filename, const char *topic,
                                    bool want_error_header, va_list arglist);
static int ww_show_help_internal(const char *filename, const char *topic,
                                   bool want_error_header, ...);

ww_show_help_fn_t ww_show_help = ww_show_help_internal;
ww_show_vhelp_fn_t ww_show_vhelp = ww_show_vhelp_internal;


int ww_show_help_init(void)
{
    ww_output_stream_t lds;

    WW_CONSTRUCT(&lds, ww_output_stream_t);
    lds.lds_want_stderr = true;
    output_stream = ww_output_open(&lds);

    ww_argv_append_nosize(&search_dirs, ww_install_dirs.wwdatadir);

    return WW_SUCCESS;
}

int ww_show_help_finalize(void)
{
    ww_output_close(output_stream);
    output_stream = -1;

    /* destruct the search list */
    if (NULL != search_dirs) {
        ww_argv_free(search_dirs);
        search_dirs = NULL;
    };
...
static int open_file(const char *base, const char *topic)
{
    ...
    if (NULL != search_dirs) {
        ...
        for (i=0; NULL != search_dirs[i]; i++) {
            filename = ww_os_path( false, search_dirs[i], base, NULL );
            ww_show_help_yyin = fopen(filename, "r");
            if (NULL == ww_show_help_yyin) {
                asprintf(&err_msg, "%s: %s", filename, strerror(errno));
                base_len = strlen(base);
                if (4 > base_len || 0 != strcmp(base + base_len - 4, ".txt")) {
                    free(filename);
                    asprintf(&filename, "%s%s%s.txt", search_dirs[i], WW_PATH_SEP, base);
                    ww_show_help_yyin = fopen(filename, "r");
                }
            }
            free(filename);
    ...
    if (NULL == ww_show_help_yyin) {
        ww_output(output_stream, "%sSorry!  You were supposed to get help about:\n    %s\nBut I couldn't open the help file:\n    %s.  Sorry!\n%s", dash_line, topic, err_msg, dash_line);
        free(err_msg);
    ...
    ww_show_help_init_buffer(ww_show_help_yyin);
...
static int find_topic(const char *base, const char *topic)
{
    ...
    while (1) {
        token = ww_show_help_yylex();
        switch (token) {
        case WW_SHOW_HELP_PARSE_TOPIC:
            tmp = strdup(ww_show_help_yytext);
            if (NULL == tmp) {
                return WW_ERR_OUT_OF_RESOURCE;
            }
            tmp[strlen(tmp) - 1] = '\0';
        ...
        case WW_SHOW_HELP_PARSE_MESSAGE:
            break;

        case WW_SHOW_HELP_PARSE_DONE:
...
static int read_topic(char ***array)
{
    ...
    while (1) {
        ...
        switch (token) {
        case WW_SHOW_HELP_PARSE_MESSAGE:
            /* ww_argv_append_nosize does strdup(ww_show_help_yytext) */
            rc = ww_argv_append_nosize(array, ww_show_help_yytext);
            if (rc != WW_SUCCESS) {
                return rc;
            }
            break;
...
static int load_array(char ***array, const char *filename, const char *topic)
{
    ...
    if (WW_SUCCESS != (ret = open_file(filename, topic))) {
        return ret;
    }
    ...
    fclose(ww_show_help_yyin);
    ww_show_help_yylex_destroy ();
...
char *ww_show_help_vstring(const char *filename, const char *topic,
                             bool want_error_header, va_list arglist)
{
    int rc;
    ...
    /* Load the message */
    if (WW_SUCCESS != (rc = load_array(&array, filename, topic))) {
        return NULL;
    }
    ...
    ww_argv_free(array);
    return (WW_SUCCESS == rc) ? output : NULL;
}

char *ww_show_help_string(const char *filename, const char *topic,
                            bool want_error_handler, ...)
{
    ...
    output = ww_show_help_vstring(filename, topic, want_error_handler,
                                    arglist);
    va_end(arglist);
                                    ...
                                    bool want_error_header, va_list arglist)
{
    ...
    output = ww_show_help_vstring(filename, topic, want_error_header,
                                    arglist);

    /* If we got a single string, output it with formatting */
    if (NULL != output) {
        ww_output(output_stream, "%s", output);
        free(output);
    }

    return (NULL == output) ? WW_ERROR : WW_SUCCESS;
}
                                   ...
                                   bool want_error_header, ...)
{
    ...
    rc = ww_show_vhelp(filename, topic, want_error_header, arglist);
    va_end(arglist);

    return rc;
}

int ww_show_help_add_dir(const char *directory)
{
    ww_argv_append_nosize(&search_dirs, directory);
    return WW_SUCCESS;
}
