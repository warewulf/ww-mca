...
static const char *path_sep = WW_PATH_SEP;

char *ww_os_path(bool relative, ...)
{
    va_list ap;
    char *element, *path;
    ...
    if (total_length > WW_PATH_MAX) {  /* path length is too long - reject it */
    	return(NULL);
    }
    ...
    va_end(ap);
    return ww_make_filename_os_friendly(path);
}
