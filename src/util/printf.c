...
#if WW_ENABLE_DEBUG
                    ww_output(0, "WW DEBUG WARNING: Got a NULL argument to ww_vasprintf!\n");
#endif
                    len += 5;
                }
                break;
...
int ww_asprintf(char **ptr, const char *fmt, ...)
{
    int length;
    va_list ap;

    va_start(ap, fmt);
    length = ww_vasprintf(ptr, fmt, ap);
    va_end(ap);
...
int ww_vasprintf(char **ptr, const char *fmt, va_list ap)
{
    int length;
    va_list ap2;
...
#if WW_HAVE_VA_COPY
    va_copy(ap2, ap);
#elif WW_HAVE_UNDERSCORE_VA_COPY
    __va_copy(ap2, ap);
#else
...
#if WW_HAVE_VA_COPY || WW_HAVE_UNDERSCORE_VA_COPY
    va_end(ap2);
#endif  /* WW_HAVE_VA_COPY || WW_HAVE_UNDERSCORE_VA_COPY */
...
int ww_snprintf(char *str, size_t size, const char *fmt, ...)
{
    int length;
    va_list ap;

    va_start(ap, fmt);
    length = ww_vsnprintf(str, size, fmt, ap);
    va_end(ap);
...
int ww_vsnprintf(char *str, size_t size, const char *fmt, va_list ap)
{
    int length;
    char *buf;

    length = ww_vasprintf(&buf, fmt, ap);
    if (length < 0) {
        return length;
    }
...
int main(int argc, char *argv[])
{
    ...
    length = ww_snprintf(NULL, 0, "this is a string %d", 1004);
    printf("length = %d\n", length);

    puts("test of snprintf to an undersize buffer:");
    length = ww_snprintf(a, sizeof(a), "this is a string %d", 1004);
    printf("string = %s\n", a);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(a));

    puts("test of snprintf to an oversize buffer:");
    length = ww_snprintf(b, sizeof(b), "this is a string %d", 1004);
    printf("string = %s\n", b);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(b));

    puts("test of asprintf:");
    length = ww_asprintf(&s, "this is a string %d", 1004);
    printf("string = %s\n", s);
    printf("length = %d\n", length);
    printf("strlen = %d\n", (int) strlen(s));
