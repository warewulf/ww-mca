...
/** @file
 * WW output stream facility.
 *
 * The WW output stream facility is used to send output from the WW
 ...
 * ww_output_open() function call, and then sent output via
 * ww_output_verbose(), WW_OUTPUT(), and ww_output().  Streams are
 * closed with ww_output_close().
 *
 ...
 * - file
 *
 * Which outputs to use are specified during ww_output_open().
 *
 ...
 *
 * ww_output_open() returns an integer handle that is used in
 * successive calls to WW_OUTPUT() and ww_output() to send output to
 * the stream.
 *
 * The default "verbose" stream is opened after invoking
 * ww_output_init() (and closed after invoking
 * ww_output_finalize()).  This stream outputs to stderr only, and
 ...
 * output (both from ww_output and stdout/stderr of processes) to
 * syslog:
 *
 * WW_OUTPUT_REDIRECT - set to "syslog" to redirect to syslog. Other
 *                        options may someday be supported
 * WW_OUTPUT_SYSLOG_PRI - set to "info", "error", or "warn" to have
 *                        output sent to syslog at that priority
 * WW_OUTPUT_SYSLOG_IDENT - a string identifier for the log
 *
 ...
 */
WW_DECLSPEC extern bool ww_output_redirected_to_syslog;
WW_DECLSPEC extern int ww_output_redirected_syslog_pri;

/**
 * \class ww_output_stream_t
 *
 * Structure used to request the opening of a WW output stream.  A
 * pointer to this structure is passed to ww_output_open() to tell
 * the ww_output subsystem where to send output for a given stream.
 ...
 * destinations through the ww_output facility.
 *
...
struct ww_output_stream_t {
     ...
     * ww_output_verbose() function.  Verbose levels are ignored in
     * WW_OUTPUT() and ww_output().
     *
    ...
    /**
     * When ww_output_stream_t::lds_want_syslog is true, this field is
     ...
     *
     * If a NULL value is given, the string "ww" is used.
     */
     ...
     * to an internal structure in the call to ww_output_open()!
     */
    char *lds_prefix;
     ...
     * the stream unless WW was configured with --enable-debug.
     */
    ...
    bool lds_want_file;
    /**
     * When ww_output_stream_t::lds_want_file is true, this field
     ...
     * prefix set by ww_output_set_output_file_info() (e.g.,
     ...
     * exist when ww_output_open() is invoked.  See ww_output()
     ...
     * Convenience typedef
     */
    typedef struct ww_output_stream_t ww_output_stream_t;
     ...
     * and can be written to via calls to ww_output_verbose() and
     * ww_output_error().
     *
     * By definition, the default verbose stream has a handle ID of 0,
     * and has a verbose level of 0.
     */
    WW_DECLSPEC bool ww_output_init(void);
     ...
     * stream.
     */
    WW_DECLSPEC void ww_output_finalize(void);

    /**
     * Opens an output stream.
     *
     * @param lds A pointer to ww_output_stream_t describing what the
     ...
     * using it in successive calls to WW_OUTPUT(), ww_output(),
     * ww_output_switch(), and ww_output_close().
     *
     ...
     * unspecified manner.
     *
     * Be sure to see ww_output() for a description of what happens
     * when open_open() / ww_output() is directed to send output to a
     * file but the process session directory does not yet exist.
     */
    WW_DECLSPEC int ww_output_open(ww_output_stream_t *lds);
     ...
     */
    WW_DECLSPEC int ww_output_reopen(int output_id, ww_output_stream_t *lds);
     ...
     * the ww_output subsystem to intercept and discard any output sent
     * to the stream via WW_OUTPUT() or ww_output() until the output
     * is re-enabled.
     */
    WW_DECLSPEC bool ww_output_switch(int output_id, bool enable);
     ...
     */
    WW_DECLSPEC void ww_output_reopen_all(void);
     ...
     */
    WW_DECLSPEC void ww_output_close(int output_id);

    /**
     * Main function to send output to a stream.
     *
     * @param output_id Stream id returned from ww_output_open().
     ...
     * ww_output_verbose() and ww_output_error()).
     *
     ...
     * session directory does not exist when ww_output() is invoked,
     * the output will be discarded!  Once the session directory is
     * created, ww_output() will automatically create the file and
     * writing to it.
     */
    WW_DECLSPEC void ww_output(int output_id, const char *format, ...) __ww_attribute_format__(__printf__, 2, 3);
     ...
     * function will effectively invoke ww_output to send the output to
     * the stream.
     *
     * @see ww_output_set_verbosity()
     */
    WW_DECLSPEC void ww_output_verbose(int verbose_level, int output_id,
                                           const char *format, ...) __ww_attribute_format__(__printf__, 3, 4);

   /**
    * Same as ww_output_verbose(), but takes a va_list form of varargs.
    */
    WW_DECLSPEC void ww_output_vverbose(int verbose_level, int output_id,
                                            const char *format, va_list ap) __ww_attribute_format__(__printf__, 3, 0);
     ...
     * specified by the format parameter.
     *
     * Exactly the same as ww_output_verbose(), except the output it
     ...
     */
    WW_DECLSPEC char *ww_output_string(int verbose_level, int output_id,
                                           const char *format, ...) __ww_attribute_format__(__printf__, 3, 4);

   /**
    * Same as ww_output_string, but accepts a va_list form of varargs.
    */
    WW_DECLSPEC char *ww_output_vstring(int verbose_level, int output_id,
                                            const char *format, va_list ap) __ww_attribute_format__(__printf__, 3, 0);
     ...
     * will be used for all future invocations of ww_output_verbose().
     */
    WW_DECLSPEC void ww_output_set_verbosity(int output_id, int level);
     ...
     */
    WW_DECLSPEC int ww_output_get_verbosity(int output_id);
     ...
     * ww_output_stream_t::lds_want_file is true, the output
     ...
     * after ww_output_set_output_file_info() returns.
     *
     ...
     * to the new directory).  More specifically, the ww_output
     ...
     */
    WW_DECLSPEC void ww_output_set_output_file_info(const char *dir,
                                                        const char *prefix,
                                                        char **olddir,
                                                        char **oldprefix);

    /**
     * Same as ww_output_verbose(), but pointer to buffer and size.
     */
    WW_DECLSPEC void ww_output_hexdump(int verbose_level, int output_id,
                                           void *ptr, int buflen);
     ...
     * @see ww_output()
     */
#define WW_OUTPUT(a) ww_output a

    /**
     * Macro for use in sending debugging output to the output
     * streams.  Will be "compiled out" when WW is configured
     * without --enable-debug.
     *
     * @see ww_output_verbose()
     */
#define WW_OUTPUT_VERBOSE(a) ww_output_verbose a
#else
    /**
     * Main macro for use in sending debugging output to output streams;
     * will be "compiled out" when WW is configured without
 ...
 */
WW_DECLSPEC WW_CLASS_DECLARATION(ww_output_stream_t);
