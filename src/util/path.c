...
bool ww_path_is_absolute( const char *path )
{
    if( WW_PATH_SEP[0] == *path ) {
        return true;
    }
    return false;
}
 ...
 */
char *ww_path_find(char *fname, char **pathv, int mode, char **envv)
{
    char *fullpath;
    char *delimit;
    char *env;
    ...
    if( ww_path_is_absolute(fname) ) {
        return ww_path_access(fname, NULL, mode);
    }
    ...
    while (pathv[i] && NULL == fullpath) {
        ...
        if ('$' == *pathv[i]) {
            delimit = strchr(pathv[i], WW_PATH_SEP[0]);
            if (delimit) {
                *delimit = '\0';
            }
            env = list_env_get(pathv[i]+1, envv);
            if (delimit) {
                *delimit = WW_PATH_SEP[0];
            }
            if (NULL != env) {
                if (!delimit) {
                    fullpath = ww_path_access(fname, env, mode);
                } else {
                    ...
                    strcpy(pfix, env);
                    strcat(pfix, delimit);
                    fullpath = ww_path_access(fname, pfix, mode);
                    free(pfix);
                }
            }
        }
        else {
            fullpath = ww_path_access(fname, pathv[i], mode);
        }
        i++;
    }
    return ww_make_filename_os_friendly(fullpath);
}

/*
 * Locates a file with certain permissions from a list of search paths
 */
char *ww_path_findv(char *fname, int mode, char **envv, char *wrkdir)
{
    char **dirv;
    char *fullpath;
    char *path;
    ...
    if (!found_dot && NULL != wrkdir) {
        ww_argv_append(&dirc, &dirv, wrkdir);
    }

    if(NULL == dirv)
        return NULL;
    fullpath = ww_path_find(fname, dirv, mode, envv);
    ww_argv_free(dirv);
    return fullpath;
}
 ...
 *      -NULL Failure
 */
char *ww_path_access(char *fname, char *path, int mode)
{
    char *fullpath = NULL;
    struct stat buf;
    ...
    if (NULL == path) {
        fullpath = ww_os_path(false, fname, NULL);
    } else {
        fullpath = ww_os_path(false, path, fname, NULL);
    }
    if (NULL == fullpath)
        return NULL;
...
static void path_env_load(char *path, int *pargc, char ***pargv)
{
    ...
    while ('\0' != *path) {
        ...
        if (p != path) {
            saved = *p;
            *p = '\0';
            ww_argv_append(pargc, pargv, path);
            *p = saved;
            path = p;
        }
 ...
 * will be returned.
 */
char* ww_find_absolute_path( char* app_name )
{
    char* abs_app_name;
    char cwd[WW_PATH_MAX], *pcwd;

    if( ww_path_is_absolute(app_name) ) { /* already absolute path */
        abs_app_name = app_name;
    } else if ( '.' == app_name[0] ||
               NULL != strchr(app_name, WW_PATH_SEP[0])) {
        /* the app is in the current directory or below it */
        pcwd = getcwd( cwd, WW_PATH_MAX );
        if( NULL == pcwd ) {
            /* too bad there is no way we can get the app absolute name */
            return NULL;
        }
        abs_app_name = ww_os_path( false, pcwd, app_name, NULL );
    } else {
        /* Otherwise try to search for the application in the PATH ... */
        abs_app_name = ww_path_findv( app_name, X_OK, NULL, NULL );
    }

    if( NULL != abs_app_name ) {
        char* resolved_path = (char*)malloc(WW_PATH_MAX);
...
static char *ww_check_mtab(char *dev_path)
{
...
bool ww_path_nfs(char *fname, char **ret_fstype)
{
    int i;
    int fsrc = -1;
    int vfsrc = -1;
...
#endif
    /*
     * Be sure to update the test (test/util/ww_path_nfs.c)
...
again:
    ...
    if (-1 == fsrc && -1 == vfsrc) {
        ...
        WW_OUTPUT_VERBOSE((10, 0, "ww_path_nfs: stat(v)fs on file:%s failed errno:%d directory:%s\n",
                             fname, errno, file));
        if (EPERM == errno) {
            free(file);
            if ( NULL != ret_fstype ) {
...
#    endif
  ...
  found:
    ...
    if (AUTOFS_SUPER_MAGIC == fs_types[i].f_fsid) {
        char *fs_type = ww_check_mtab(fname);
        int x;
        if (NULL != fs_type) {
            for (x = 0; x < FS_TYPES_NUM; x++) {
                ...
                if (0 == strcasecmp(fs_types[x].f_fsname, fs_type)) {
                    WW_OUTPUT_VERBOSE((10, 0, "ww_path_nfs: file:%s on fs:%s\n", fname, fs_type));
                    free(fs_type);
                    if ( NULL != ret_fstype ) {
             ...
             uint64_t *out_avail)
{
    ...
    *out_avail = 0;
    ...
    if (-1 == rc) {
        WW_OUTPUT_VERBOSE((10, 2, "ww_path_df: stat(v)fs on "
    ...
    *out_avail = buf.f_bsize * ((int)buf.f_bavail < 0 ? 0 : buf.f_bavail);
    ...
    WW_OUTPUT_VERBOSE((10, 2, "ww_path_df: stat(v)fs states "
