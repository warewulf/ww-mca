 ...
 */
static inline char* ww_find_last_path_separator( const char* filename, size_t n )
{
...
char *ww_basename(const char *filename)
{
    size_t i;
    char *tmp, *ret = NULL;
    const char sep = WW_PATH_SEP[0];
    ...
    ret = ww_find_last_path_separator( tmp, strlen(tmp) );
    if (NULL == ret) {
        return tmp;
    }
    ret = strdup(ret + 1);
...
char* ww_dirname(const char* filename)
{
#if defined(HAVE_DIRNAME) || WW_HAVE_DIRNAME
    char* safe_tmp = strdup(filename), *result;
    result = strdup(dirname(safe_tmp));
    free(safe_tmp);
    return result;
#else
    const char* p = ww_find_last_path_separator(filename, strlen(filename));
...
#endif
                ret[p - filename] = '\0';
                return ww_make_filename_os_friendly(ret);
            }
            break;  /* return the duplicate of "." */
        }
    }
    return strdup(".");
#endif  /* defined(HAVE_DIRNAME) || WW_HAVE_DIRNAME */
}
