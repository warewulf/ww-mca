 ...
 */
ww_status_t ww_argv_append(int *argc, char ***argv, const char *arg)
{
    ww_status_t rc;

    /* add the new element */
    if (WW_SUCCESS != (rc = ww_argv_append_nosize(argv, arg))) {
        return rc;
    }

    *argc = ww_argv_count(*argv);

    return WW_SUCCESS;
}

ww_status_t ww_argv_append_nosize(char ***argv, const char *arg)
{
    int argc;
...
ww_status_t ww_argv_prepend_nosize(char ***argv, const char *arg)
{
    int argc;
    int i;
...
ww_status_t ww_argv_append_unique_nosize(char ***argv, const char *arg, bool overwrite)
{
    int i;
     ...
     */
    if (NULL == *argv) {
        return ww_argv_append_nosize(argv, arg);
    }
 ...
 */
static char **ww_argv_split_inter(const char *src_string, int delimiter,
        int include_empty)
{
  char arg[ARGSIZE];
  char **argv = NULL;
  ...
  while (src_string && *src_string) {
    ...
    if (src_string == p) {
      if (include_empty) {
        arg[0] = '\0';
        if (WW_SUCCESS != ww_argv_append(&argc, &argv, arg))
          return NULL;
      }
    }
    ...
    /* tail argument, add straight from the original string */
      ...
      if (WW_SUCCESS != ww_argv_append(&argc, &argv, src_string))
	return NULL;
      src_string = p;
      continue;
    }
	...
	return NULL;
      ...
      if (WW_SUCCESS != ww_argv_append(&argc, &argv, argtemp)) {
	free(argtemp);
	return NULL;
      }
...
char **ww_argv_split(const char *src_string, int delimiter)
{
    return ww_argv_split_inter(src_string, delimiter, 0);
}

char **ww_argv_split_with_empty(const char *src_string, int delimiter)
{
    return ww_argv_split_inter(src_string, delimiter, 1);
}

/*
 * Return the length of a NULL-terminated argv array.
 */
int ww_argv_count(char **argv)
{
  char **p;
  int i;
 ...
 */
char *ww_argv_join(char **argv, int delimiter)
{
  char **p;
  char *pp;
  char *str;
 ...
 */
char *ww_argv_join_range(char **argv, size_t start, size_t end, int delimiter)
{
    char **p;
    char *pp;
    char *str;
    ...
    if (NULL == argv || NULL == argv[0] || (int)start > ww_argv_count(argv)) {
        return strdup("");
    }
...
char **ww_argv_copy(char **argv)
{
  ...
  while (NULL != *argv) {
    if (WW_SUCCESS != ww_argv_append(&dupc, &dupv, *argv)) {
      ww_argv_free(dupv);
      return NULL;
    }
...
ww_status_t ww_argv_delete(int *argc, char ***argv, int start, int num_to_delete)
{
    int i;
    int count;
    int suffix_count;
...
ww_status_t ww_argv_insert(char ***target, int start, char **source)
{
    ...
    target_count = ww_argv_count(*target);
    source_count = ww_argv_count(source);
    if (start > target_count) {
        for (i = 0; i < source_count; ++i) {
            ww_argv_append(&target_count, target, source[i]);
        }
    }
...
ww_status_t ww_argv_insert_element(char ***target, int location, char *source)
{
    int i, target_count;
    int suffix_count;
    ...
    if (location > target_count) {
        ww_argv_append(&target_count, target, source);
        return WW_SUCCESS;
    }
