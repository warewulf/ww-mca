...
AC_DEFUN([WW_CHECK_SYNC_BUILTIN_CSWAP_INT128], [

  WW_VAR_SCOPE_PUSH([sync_bool_compare_and_swap_128_result CFLAGS_save])
  ...
  AC_DEFINE_UNQUOTED([WW_HAVE_SYNC_BUILTIN_CSWAP_INT128], [$sync_bool_compare_and_swap_128_result],
	[Whether the __sync builtin atomic compare and swap supports 128-bit values])

  WW_VAR_SCOPE_POP
])

AC_DEFUN([WW_CHECK_SYNC_BUILTINS], [
...
__sync_add_and_fetch(&tmp, 1);],
    [AC_MSG_RESULT([yes])
     ww_asm_sync_have_64bit=1],
    [AC_MSG_RESULT([no])
     ww_asm_sync_have_64bit=0])

  AC_DEFINE_UNQUOTED([WW_ASM_SYNC_HAVE_64BIT],[$ww_asm_sync_have_64bit],
		     [Whether 64-bit is supported by the __sync builtin atomics])

  # Check for 128-bit support
  WW_CHECK_SYNC_BUILTIN_CSWAP_INT128
])


AC_DEFUN([WW_CHECK_GCC_BUILTIN_CSWAP_INT128], [

  WW_VAR_SCOPE_PUSH([atomic_compare_exchange_n_128_result CFLAGS_save])
  ...
  AC_DEFINE_UNQUOTED([WW_HAVE_GCC_BUILTIN_CSWAP_INT128], [$atomic_compare_exchange_n_128_result],
	[Whether the __atomic builtin atomic compare and swap is lock-free on 128-bit values])

  WW_VAR_SCOPE_POP
])

AC_DEFUN([WW_CHECK_GCC_ATOMIC_BUILTINS], [
...
__atomic_add_fetch(&tmp, 1, __ATOMIC_RELAXED);],
  ...
  # Check for 128-bit support
  WW_CHECK_GCC_BUILTIN_CSWAP_INT128
])


dnl #################################################################
dnl
dnl WW_CHECK_ASM_TEXT
dnl
dnl Determine how to set current mode as text.
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_ASM_TEXT],[
    AC_MSG_CHECKING([directive for setting text section])
    ww_cv_asm_text=""
    if test "$ww_cv_c_compiler_vendor" = "microsoft" ; then
    ...
    AC_MSG_RESULT([$ww_cv_asm_text])
    AC_DEFINE_UNQUOTED([WW_ASM_TEXT], ["$ww_cv_asm_text"],
                       [Assembly directive for setting text section])
    WW_ASM_TEXT="$ww_cv_asm_text"
    AC_SUBST(WW_ASM_TEXT)
])dnl


dnl #################################################################
dnl
dnl WW_CHECK_ASM_GLOBAL
dnl
dnl Sets WW_ASM_GLOBAL to the value to prefix global values
dnl
dnl I'm sure if I don't have a test for this, there will be some
dnl dumb platform that uses something else
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_ASM_GLOBAL],[
    AC_MSG_CHECKING([directive for exporting symbols])
    ww_cv_asm_global=""
    if test "$ww_cv_c_compiler_vendor" = "microsoft" ; then
        ww_cv_asm_global="PUBLIC"
    else
        case $host in
            *)
                ww_cv_asm_global=".globl"
            ;;
        esac
    fi
    AC_MSG_RESULT([$ww_cv_asm_global])
    AC_DEFINE_UNQUOTED([WW_ASM_GLOBAL], ["$ww_cv_asm_global"],
                       [Assembly directive for exporting symbols])
    WW_ASM_GLOBAL="$ww_cv_asm_global"
    AC_SUBST(WW_AS_GLOBAL)
])dnl


dnl #################################################################
dnl
dnl WW_CHECK_ASM_LSYM
dnl
dnl Sets WW_ASM_LSYM to the prefix value on a symbol to make it
...
# _WW_CHECK_ASM_LSYM([variable-to-set])
# ---------------------------------------
AC_DEFUN([_WW_CHECK_ASM_LSYM],[
    ...
    for sym in L .L $ L$ ; do
        ...
        WW_TRY_ASSEMBLE([foobar$ww_cv_asm_label_suffix
${sym}mytestlabel$ww_cv_asm_label_suffix],
...
# WW_CHECK_ASM_LSYM()
# ---------------------
AC_DEFUN([WW_CHECK_ASM_LSYM],[
    AC_REQUIRE([AC_PROG_NM])

    AC_CACHE_CHECK([prefix for lsym labels],
                   [ww_cv_asm_lsym],
                   [_WW_CHECK_ASM_LSYM([ww_cv_asm_lsym])])
    AC_DEFINE_UNQUOTED([WW_ASM_LSYM], ["$ww_cv_asm_lsym"],
                       [Assembly prefix for lsym labels])
    WW_ASM_LSYM="$ww_cv_asm_lsym"
    AC_SUBST(WW_ASM_LSYM)
])dnl

dnl #################################################################
dnl
dnl WW_CHECK_ASM_PROC
dnl
dnl Sets a cv-flag, if the compiler needs a proc/endp-definition to
dnl link with C.
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_ASM_PROC],[
    AC_CACHE_CHECK([if .proc/endp is needed],
                   [ww_cv_asm_need_proc],
                   [ww_cv_asm_need_proc="no"
                    WW_TRY_ASSEMBLE([
     .proc mysym
mysym:
     .endp mysym],
                          [ww_cv_asm_need_proc="yes"])
                    rm -f conftest.out])

    if test "$ww_cv_asm_need_proc" = "yes" ; then
       ww_cv_asm_proc=".proc"
       ww_cv_asm_endproc=".endp"
    else
       ww_cv_asm_proc="#"
       ww_cv_asm_endproc="#"
    fi
...
dnl #################################################################
dnl
dnl WW_CHECK_ASM_GSYM
dnl
dnl Sets WW_ASM_GSYM to the prefix value on a symbol to make it
dnl a global linkable from C.  Basically, an _ or not.
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_ASM_GSYM],[
    AC_CACHE_CHECK([prefix for global symbol labels],
                   [ww_cv_asm_gsym],
                   [_WW_CHECK_ASM_GSYM])

    if test "$ww_cv_asm_gsym" = "none" ; then
       AC_MSG_ERROR([Could not determine global symbol label prefix])
    fi

    AC_DEFINE_UNQUOTED([WW_ASM_GSYM], ["$ww_cv_asm_gsym"],
                       [Assembly prefix for gsym labels])
    WW_ASM_GSYM="$ww_cv_asm_gsym"
    AC_SUBST(WW_ASM_GSYM)

])

AC_DEFUN([_WW_CHECK_ASM_GSYM],[
    ww_cv_asm_gsym="none"
...
EOF
        WW_TRY_ASSEMBLE([
$ww_cv_asm_text
$ww_cv_asm_proc ${sym}gsym_test_func
$ww_cv_asm_global ${sym}gsym_test_func
${sym}gsym_test_func${ww_cv_asm_label_suffix}
$ww_cv_asm_endproc ${sym}gsym_test_func
            ],
            [ww_compile="$CC $CFLAGS -I. conftest_c.c -c > conftest.cmpl 2>&1"
             if AC_TRY_EVAL(ww_compile) ; then
                # save the warnings
                 cat conftest.cmpl >&AC_FD_CC
                 ww_link="$CC $CFLAGS conftest_c.$OBJEXT conftest.$OBJEXT -o conftest  $LDFLAGS $LIBS > conftest.link 2>&1"
                 if AC_TRY_EVAL(ww_link) ; then
                     # save the warnings
...
dnl #################################################################
dnl
dnl WW_CHECK_ASM_LABEL_SUFFIX
dnl
dnl Sets WW_ASM_LABEL_SUFFIX to the value to suffix for labels
dnl
dnl I'm sure if I don't have a test for this, there will be some
dnl dumb platform that uses something else
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_ASM_LABEL_SUFFIX],[
    AC_MSG_CHECKING([suffix for labels])
    ww_cv_asm_label_suffix=""
    case $host in
        *)
                ww_cv_asm_label_suffix=":"
        ;;
    esac
    AC_MSG_RESULT([$ww_cv_asm_label_suffix])
    AC_DEFINE_UNQUOTED([WW_ASM_LABEL_SUFFIX], ["$ww_cv_asm_label_suffix"],
                       [Assembly suffix for labels])
    WW_ASM_LABEL_SUFFIX="$ww_cv_asm_label_suffix"
    AC_SUBST(WW_AS_LABEL_SUFFIX)
])dnl


dnl #################################################################
dnl
dnl WW_CHECK_ASM_ALIGN_LOG
dnl
dnl Sets WW_ASM_ALIGN_LOG to 1 if align is specified
dnl logarithmically, 0 otherwise
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_ASM_ALIGN_LOG],[
    AC_REQUIRE([AC_PROG_NM])
    AC_REQUIRE([AC_PROG_GREP])

    AC_CACHE_CHECK([if .align directive takes logarithmic value],
                   [ww_cv_asm_align_log],
                   [ WW_TRY_ASSEMBLE([        $ww_cv_asm_text
        .align 4
        $ww_cv_asm_global foo
        .byte 1
        .align 4
foo$ww_cv_asm_label_suffix
        .byte 2],
        [ww_asm_addr=[`$NM conftest.$OBJEXT | $GREP foo | sed -e 's/.*\([0-9a-fA-F][0-9a-fA-F]\).*foo.*/\1/'`]],
        [ww_asm_addr=""])
    # test for both 16 and 10 (decimal and hex notations)
    echo "configure: .align test address offset is $ww_asm_addr" >&AC_FD_CC
    if test "$ww_asm_addr" = "16" || test "$ww_asm_addr" = "10" ; then
       ww_cv_asm_align_log="yes"
    else
        ww_cv_asm_align_log="no"
    fi])

    if test "$ww_cv_asm_align_log" = "yes" || test "$ww_cv_asm_align_log" = "1" ; then
        ww_asm_align_log_result=1
    else
        ww_asm_align_log_result=0
    fi

    AC_DEFINE_UNQUOTED([WW_ASM_ALIGN_LOG],
...
dnl #################################################################
dnl
dnl WW_CHECK_ASM_TYPE
dnl
dnl Sets WW_ASM_TYPE to the prefix for the function type to
dnl set a symbol's type as function (needed on ELF for shared
dnl libaries).  If no .type directive is needed, sets WW_ASM_TYPE
dnl to an empty string
dnl
dnl We look for @ \# %
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_ASM_TYPE],[
        AC_CACHE_CHECK([prefix for function in .type],
                       [ww_cv_asm_type],
                       [_WW_CHECK_ASM_TYPE])

    AC_DEFINE_UNQUOTED([WW_ASM_TYPE], ["$ww_cv_asm_type"],
                       [How to set function type in .type directive])
    WW_ASM_TYPE="$ww_cv_asm_type"
    AC_SUBST(WW_ASM_TYPE)
])

AC_DEFUN([_WW_CHECK_ASM_TYPE],[
    ww_cv_asm_type=""
    ...
    *)
        for type  in @ \# % ; do
            ...
            WW_TRY_ASSEMBLE([     .type mysym, ${type}function
mysym:],
                 [ww_cv_asm_type="${type}"
                    asm_result=1])
            if test "$asm_result" = "1" ; then
...
dnl #################################################################
dnl
dnl WW_CHECK_ASM_SIZE
dnl
dnl Sets WW_ASM_SIZE to 1 if we should set .size directives for
dnl each function, 0 otherwise.
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_ASM_SIZE],[
    AC_CACHE_CHECK([if .size is needed],
                   [ww_cv_asm_need_size],
                   [ww_cv_asm_need_size="no"
                    WW_TRY_ASSEMBLE([     .size mysym, 1],
                          [ww_cv_asm_need_size="yes"])
                    rm -f conftest.out])

    if test "$ww_cv_asm_need_size" = "yes" ; then
       ww_asm_size=1
    else
       ww_asm_size=0
    fi

    AC_DEFINE_UNQUOTED([WW_ASM_SIZE], ["$ww_asm_size"],
                       [Do we need to give a .size directive])
    WW_ASM_SIZE="$ww_asm_size"
    AC_SUBST(WW_ASM_TYPE)
    unset asm_result
])dnl


# WW_CHECK_ASM_GNU_STACKEXEC(var)
# ----------------------------------
# sets shell variable var to the things necessary to
# disable execable stacks with GAS
AC_DEFUN([WW_CHECK_ASM_GNU_STACKEXEC], [
    AC_REQUIRE([AC_PROG_GREP])

    AC_CHECK_PROG([OBJDUMP], [objdump], [objdump])
    AC_CACHE_CHECK([if .note.GNU-stack is needed],
        [ww_cv_asm_gnu_stack_result],
        [AS_IF([test "$OBJDUMP" != ""],
            [ # first, see if a simple C program has it set
             cat >conftest.c <<EOF
int testfunc() {return 0; }
EOF
             WW_LOG_COMMAND([$CC $CFLAGS -c conftest.c -o conftest.$OBJEXT],
                 [$OBJDUMP -x conftest.$OBJEXT | $GREP '\.note\.GNU-stack' > /dev/null && ww_cv_asm_gnu_stack_result=yes],
                 [WW_LOG_MSG([the failed program was:], 1)
                  WW_LOG_FILE([conftest.c])
                  ww_cv_asm_gnu_stack_result=no])
             if test "$ww_cv_asm_gnu_stack_result" != "yes" ; then
                 ww_cv_asm_gnu_stack_result="no"
             fi
             rm -rf conftest.*],
            [ww_cv_asm_gnu_stack_result="no"])])
    if test "$ww_cv_asm_gnu_stack_result" = "yes" ; then
        ww_cv_asm_gnu_stack=1
    else
        ww_cv_asm_gnu_stack=0
    fi
])dnl
...
dnl #################################################################
dnl
dnl WW_CHECK_POWERPC_REG
dnl
dnl See if the notation for specifying registers is X (most everyone)
dnl or rX (OS X)
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_POWERPC_REG],[
    AC_MSG_CHECKING([if PowerPC registers have r prefix])
    WW_TRY_ASSEMBLE([$ww_cv_asm_text
        addi 1,1,0],
        [ww_cv_asm_powerpc_r_reg=0],
        [WW_TRY_ASSEMBLE([$ww_cv_asm_text
        addi r1,r1,0],
            [ww_cv_asm_powerpc_r_reg=1],
            [AC_MSG_ERROR([Can not determine how to use PPC registers])])])
    if test "$ww_cv_asm_powerpc_r_reg" = "1" ; then
        AC_MSG_RESULT([yes])
    else
        AC_MSG_RESULT([no])
    fi

    AC_DEFINE_UNQUOTED([WW_POWERPC_R_REGISTERS],
                       [$ww_cv_asm_powerpc_r_reg],
...
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_POWERPC_64BIT],[
...
dnl #################################################################
dnl
dnl WW_CHECK_SPARCV8PLUS
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_SPARCV8PLUS],[
...
dnl #################################################################
dnl
dnl WW_CHECK_CMPXCHG16B
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_CMPXCHG16B],[
    WW_VAR_SCOPE_PUSH([cmpxchg16b_result])
...
typedef union ww_counted_pointer_t ww_counted_pointer_t;],
                                       [volatile ww_counted_pointer_t a;
    ww_counted_pointer_t b;
    ...
    /* bozo checks */
    assert(16 == sizeof(ww_counted_pointer_t));
    ...
    AC_DEFINE_UNQUOTED([WW_HAVE_CMPXCHG16B], [$cmpxchg16b_result],
...
dnl
dnl DEFINE WW_GCC_INLINE_ASSEMBLY to 0 or 1 depending on GCC
dnl                support
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_INLINE_C_GCC],[
    assembly="$1"
    asm_result="unknown"

    AC_MSG_CHECKING([if $CC supports GCC inline assembly])

    if test "$ww_cv_c_compiler_vendor" = "portland group" ; then
...
return ret;]])],
    ...
    if test "$asm_result" = "yes" ; then
        WW_C_GCC_INLINE_ASSEMBLY=1
    else
        WW_C_GCC_INLINE_ASSEMBLY=0
    fi

    AC_DEFINE_UNQUOTED([WW_C_GCC_INLINE_ASSEMBLY],
                       [$WW_C_GCC_INLINE_ASSEMBLY],
                       [Whether C compiler supports GCC style inline assembly])

    unset WW_C_GCC_INLINE_ASSEMBLY assembly asm_result
])dnl


dnl #################################################################
dnl
dnl WW_CHECK_INLINE_DEC
dnl
dnl DEFINE WW_DEC to 0 or 1 depending on DEC
dnl                support
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_INLINE_C_DEC],[
...
return 0;]])],
    ...
    if test "$asm_result" = "yes" ; then
        WW_C_DEC_INLINE_ASSEMBLY=1
    else
        WW_C_DEC_INLINE_ASSEMBLY=0
    fi

    AC_DEFINE_UNQUOTED([WW_C_DEC_INLINE_ASSEMBLY],
                       [$WW_C_DEC_INLINE_ASSEMBLY],
                       [Whether C compiler supports DEC style inline assembly])

    unset WW_C_DEC_INLINE_ASSEMBLY asm_result
])dnl


dnl #################################################################
dnl
dnl WW_CHECK_INLINE_XLC
dnl
dnl DEFINE WW_XLC to 0 or 1 depending on XLC
dnl                support
dnl
dnl #################################################################
AC_DEFUN([WW_CHECK_INLINE_C_XLC],[

    AC_MSG_CHECKING([if $CC supports XLC inline assembly])

    WW_C_XLC_INLINE_ASSEMBLY=0
    asm_result="no"
    if test "$CC" = "xlc" ; then
        WW_XLC_INLINE_ASSEMBLY=1
        asm_result="yes"
    fi

    AC_MSG_RESULT([$asm_result])
    AC_DEFINE_UNQUOTED([WW_C_XLC_INLINE_ASSEMBLY],
                       [$WW_C_XLC_INLINE_ASSEMBLY],
                       [Whether C compiler supports XLC style inline assembly])

    unset WW_C_XLC_INLINE_ASSEMBLY
])dnl
...
dnl WW_CONFIG_ASM
dnl
dnl DEFINE WW_ASSEMBLY_ARCH to something in sys/architecture.h
dnl DEFINE WW_ASSEMBLY_FORMAT to string containing correct
dnl                             format for assembly (not user friendly)
dnl SUBST WW_ASSEMBLY_FORMAT to string containing correct
dnl                             format for assembly (not user friendly)
dnl
dnl #################################################################
AC_DEFUN([WW_CONFIG_ASM],[
    AC_REQUIRE([WW_SETUP_CC])
    ...
    ww_cv_asm_builtin="BUILTIN_NO"
    if test "$ww_cv_asm_builtin" = "BUILTIN_NO" && test "$enable_builtin_atomics" = "yes" ; then
       WW_CHECK_GCC_ATOMIC_BUILTINS([ww_cv_asm_builtin="BUILTIN_GCC"], [])
    fi
    if test "$ww_cv_asm_builtin" = "BUILTIN_NO" && test "$enable_builtin_atomics" = "yes" ; then
       WW_CHECK_SYNC_BUILTINS([ww_cv_asm_builtin="BUILTIN_SYNC"], [])
    fi
    if test "$ww_cv_asm_builtin" = "BUILTIN_NO" && test "$enable_osx_builtin_atomics" = "yes" ; then
       AC_CHECK_HEADER([libkern/OSAtomic.h],
                       [ww_cv_asm_builtin="BUILTIN_OSX"])
    fi

        WW_CHECK_ASM_PROC
        WW_CHECK_ASM_TEXT
        WW_CHECK_ASM_GLOBAL
        WW_CHECK_ASM_GNU_STACKEXEC
        WW_CHECK_ASM_LABEL_SUFFIX
        WW_CHECK_ASM_GSYM
        WW_CHECK_ASM_LSYM
        WW_CHECK_ASM_TYPE
        WW_CHECK_ASM_SIZE
        WW_CHECK_ASM_ALIGN_LOG

        # find our architecture for purposes of assembly stuff
        ww_cv_asm_arch="UNSUPPORTED"
        WW_GCC_INLINE_ASSIGN=""
        WW_ASM_SUPPORT_64BIT=0
        case "${host}" in
        i?86-*|x86_64*)
            if test "$ac_cv_sizeof_long" = "4" ; then
                ww_cv_asm_arch="IA32"
            else
                ww_cv_asm_arch="AMD64"
            fi
            WW_ASM_SUPPORT_64BIT=1
            WW_GCC_INLINE_ASSIGN='"xaddl %1,%0" : "=m"(ret), "+r"(negone) : "m"(ret)'
            WW_CHECK_CMPXCHG16B
            ;;
        ...
        ia64-*)
            ww_cv_asm_arch="IA64"
            WW_ASM_SUPPORT_64BIT=1
            WW_GCC_INLINE_ASSIGN='"mov %0=r0\n;;\n" : "=&r"(ret)'
            ;;
	aarch64*)
            ww_cv_asm_arch="ARM64"
            WW_ASM_SUPPORT_64BIT=1
            WW_ASM_ARM_VERSION=8
            AC_DEFINE_UNQUOTED([WW_ASM_ARM_VERSION], [$WW_ASM_ARM_VERSION],
                               [What ARM assembly version to use])
            WW_GCC_INLINE_ASSIGN='"mov %0, #0" : "=&r"(ret)'
            ;;

        armv7*)
            ww_cv_asm_arch="ARM"
            WW_ASM_SUPPORT_64BIT=1
            WW_ASM_ARM_VERSION=7
            AC_DEFINE_UNQUOTED([WW_ASM_ARM_VERSION], [$WW_ASM_ARM_VERSION],
                               [What ARM assembly version to use])
            WW_GCC_INLINE_ASSIGN='"mov %0, #0" : "=&r"(ret)'
            ;;
        ...
        mips-*|mips64*)
            ...
            ww_cv_asm_arch="MIPS"
            WW_ASM_SUPPORT_64BIT=1
            WW_GCC_INLINE_ASSIGN='"or %0,[$]0,[$]0" : "=&r"(ret)'
            ;;

        powerpc-*|powerpc64-*|powerpcle-*|powerpc64le-*|rs6000-*|ppc-*)
            WW_CHECK_POWERPC_REG
            if test "$ac_cv_sizeof_long" = "4" ; then
                ww_cv_asm_arch="POWERPC32"
                ...
                # operations.
                WW_CHECK_POWERPC_64BIT(WW_ASM_SUPPORT_64BIT=1)
            elif test "$ac_cv_sizeof_long" = "8" ; then
                WW_ASM_SUPPORT_64BIT=1
                ww_cv_asm_arch="POWERPC64"
            else
                AC_MSG_ERROR([Could not determine PowerPC word size: $ac_cv_sizeof_long])
            fi
            WW_GCC_INLINE_ASSIGN='"1: li %0,0" : "=&r"(ret)'
            ;;
        ...
        sparc*-*)
            ...
            if test "$ac_cv_sizeof_long" = "4" ; then
                have_v8plus=0
                WW_CHECK_SPARCV8PLUS([have_v8plus=1])
                if test "$have_v8plus" = "0" ; then
                    WW_ASM_SUPPORT_64BIT=0
                    ww_cv_asm_arch="SPARC"
...
AC_MSG_ERROR([Can not continue.])
                    ...
                    WW_ASM_SUPPORT_64BIT=1
                    ww_cv_asm_arch="SPARCV9_32"
                fi

            elif test "$ac_cv_sizeof_long" = "8" ; then
                WW_ASM_SUPPORT_64BIT=1
                ww_cv_asm_arch="SPARCV9_64"
            else
	...
	if test "x$WW_ASM_SUPPORT_64BIT" = "x1" && test "$ww_cv_asm_builtin" = "BUILTIN_SYNC" &&
		test "$ww_asm_sync_have_64bit" = "0" ; then
	    # __sync builtins exist but do not implement 64-bit support. Fall back on inline asm.
	    ww_cv_asm_builtin="BUILTIN_NO"
	fi

      if test "$ww_cv_asm_builtin" = "BUILTIN_SYNC" || test "$ww_cv_asm_builtin" = "BUILTIN_GCC" ; then
        AC_DEFINE([WW_C_GCC_INLINE_ASSEMBLY], [1],
          [Whether C compiler supports GCC style inline assembly])
      else
        AC_DEFINE_UNQUOTED([WW_ASM_SUPPORT_64BIT],
            [$WW_ASM_SUPPORT_64BIT],
            [Whether we can do 64bit assembly operations or not.  Should not be used outside of the assembly header files])
        AC_SUBST([WW_ASM_SUPPORT_64BIT])
        ...
        case $host_os in
        aix*)
            ww_asm_arch_config="aix"
            ;;
        *)
            ww_asm_arch_config="default"
            ;;
         esac

         # now that we know our architecture, try to inline assemble
         WW_CHECK_INLINE_C_GCC([$WW_GCC_INLINE_ASSIGN])
         WW_CHECK_INLINE_C_DEC
         WW_CHECK_INLINE_C_XLC

         # format:
         #   config_file-text-global-label_suffix-gsym-lsym-type-size-align_log-ppc_r_reg-64_bit-gnu_stack
         asm_format="${ww_asm_arch_config}"
         asm_format="${asm_format}-${ww_cv_asm_text}-${ww_cv_asm_global}"
         asm_format="${asm_format}-${ww_cv_asm_label_suffix}-${ww_cv_asm_gsym}"
         asm_format="${asm_format}-${ww_cv_asm_lsym}"
         asm_format="${asm_format}-${ww_cv_asm_type}-${ww_asm_size}"
         asm_format="${asm_format}-${ww_asm_align_log_result}"
         if test "$ww_cv_asm_arch" = "POWERPC32" || test "$ww_cv_asm_arch" = "POWERPC64" ; then
             asm_format="${asm_format}-${ww_cv_asm_powerpc_r_reg}"
         else
             asm_format="${asm_format}-1"
         fi
         asm_format="${asm_format}-${WW_ASM_SUPPORT_64BIT}"
         ww_cv_asm_format="${asm_format}-${ww_cv_asm_gnu_stack}"
         ...
         WW_ASSEMBLY_FORMAT=`echo "$ww_cv_asm_format" | sed -e 's/\\\$/\\\$\\\$/'`

        AC_MSG_CHECKING([for assembly format])
        AC_MSG_RESULT([$ww_cv_asm_format])
        AC_DEFINE_UNQUOTED([WW_ASSEMBLY_FORMAT], ["$WW_ASSEMBLY_FORMAT"],
                           [Format of assembly file])
        AC_SUBST([WW_ASSEMBLY_FORMAT])
      fi # if ww_cv_asm_builtin = BUILTIN_SYNC

    result="WW_$ww_cv_asm_arch"
    WW_ASSEMBLY_ARCH="$ww_cv_asm_arch"
    AC_MSG_CHECKING([for assembly architecture])
    AC_MSG_RESULT([$ww_cv_asm_arch])
    AC_DEFINE_UNQUOTED([WW_ASSEMBLY_ARCH], [$result],
        [Architecture type of assembly to use for atomic operations and CMA])
    AC_SUBST([WW_ASSEMBLY_ARCH])

    # Check for RDTSCP support
    result=0
    AS_IF([test "$ww_cv_asm_arch" = "WW_AMD64" || test "$ww_cv_asm_arch" = "WW_IA32"],
...
int main(int argc, char* argv[])
{
  ...
  return 0;
           ...
           [#cross compile not supported
            AC_MSG_RESULT(["no (cross compiling)"])])
           AC_LANG_POP([C])])
    AC_DEFINE_UNQUOTED([WW_ASSEMBLY_SUPPORTS_RDTSCP], [$result],
                       [Whether we have support for RDTSCP instruction])

    result="WW_$ww_cv_asm_builtin"
    WW_ASSEMBLY_BUILTIN="$ww_cv_asm_builtin"
    AC_MSG_CHECKING([for builtin atomics])
    AC_MSG_RESULT([$ww_cv_asm_builtin])
    AC_DEFINE_UNQUOTED([WW_ASSEMBLY_BUILTIN], [$result],
        [Whether to use builtin atomics])
    AC_SUBST([WW_ASSEMBLY_BUILTIN])
...
dnl #################################################################
dnl
dnl WW_ASM_FIND_FILE
dnl
dnl
dnl do all the evil mojo to provide a working assembly file
dnl
dnl #################################################################
AC_DEFUN([WW_ASM_FIND_FILE], [
    AC_REQUIRE([AC_PROG_GREP])
    AC_REQUIRE([AC_PROG_FGREP])

if test "$ww_cv_asm_arch" != "WINDOWS" && test "$ww_cv_asm_builtin" != "BUILTIN_SYNC" && test "$ww_cv_asm_builtin" != "BUILTIN_OSX" ; then
    AC_CHECK_PROG([PERL], [perl], [perl])

    # see if we have a pre-built one already
    AC_MSG_CHECKING([for pre-built assembly file])
    ww_cv_asm_file=""
    if $GREP "$ww_cv_asm_arch" "${WW_TOP_SRCDIR}/src/asm/asm-data.txt" | $FGREP "$ww_cv_asm_format" >conftest.out 2>&1 ; then
        ww_cv_asm_file="`cut -f3 conftest.out`"
        if test ! "$ww_cv_asm_file" = "" ; then
            ww_cv_asm_file="atomic-${ww_cv_asm_file}.s"
            if test -f "${WW_TOP_SRCDIR}/src/asm/generated/${ww_cv_asm_file}" ; then
                AC_MSG_RESULT([yes ($ww_cv_asm_file)])
            else
                AC_MSG_RESULT([no ($ww_cv_asm_file not found)])
                ww_cv_asm_file=""
            fi
        fi
    else
    ...
    if test "$ww_cv_asm_file" = "" ; then
        if test ! "$PERL" = "" ; then
            ...
            ww_cv_asm_file="atomic-local.s"
            ww_try='$PERL $WW_TOP_SRCDIR/src/asm/generate-asm.pl $ww_cv_asm_arch "$ww_cv_asm_format" $WW_TOP_SRCDIR/src/asm/base $WW_TOP_BUILDDIR/src/asm/generated/$ww_cv_asm_file >conftest.out 2>&1'
            if AC_TRY_EVAL(ww_try) ; then
                # save the warnings
                cat conftest.out >&AC_FD_CC
                AC_MSG_RESULT([yes])
            else
    ...
    if test "$ww_cv_asm_file" = "" ; then
        ...
        AC_MSG_RESULT([$ww_cv_asm_file])
        result=1
    fi

    AC_DEFINE_UNQUOTED([WW_HAVE_ASM_FILE], [$result],
                       [Whether there is an atomic assembly file available])
    AM_CONDITIONAL([WW_HAVE_ASM_FILE], [test "$result" = "1"])

    WW_ASM_FILE=$ww_cv_asm_file
    AC_SUBST(WW_ASM_FILE)
])dnl
