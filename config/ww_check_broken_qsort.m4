...
dnl the ww_qsort instead.

# check for broken qsort
# WW_CHECK_BROKEN_QSORT(prefix, [action-if-found], [action-if-not-found])
# --------------------------------------------------------
AC_DEFUN([WW_CHECK_BROKEN_QSORT],[
  ...
  AC_DEFINE_UNQUOTED([WW_HAVE_BROKEN_QSORT], [$define_result],
