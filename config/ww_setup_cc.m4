...
AC_DEFUN([WW_SETUP_CC],[
    ...
    AC_REQUIRE([_WW_START_SETUP_CC])
    AC_REQUIRE([_WW_PROG_CC])
    ...
    if test "x$ac_cv_prog_cc_c99" = xno ; then
        AC_MSG_WARN([Warewulf requires a C99 compiler])
        AC_MSG_ERROR([Aborting.])
    fi


    WW_C_COMPILER_VENDOR([ww_c_vendor])
    ...
    AS_IF([test "$ww_cv_c_compiler_vendor" != "ibm"],
...
# undef _GNU_SOURCE
    ...
    if test $WANT_PICKY_COMPILER -eq 1; then
        ...
        if test "$ww_cv_cc_wno_long_double" = "yes" ; then
    ...
    if test "$GCC" = "yes"; then
        ...
        AC_CACHE_CHECK([if $CC supports -finline-functions],
                   [ww_cv_cc_finline_functions],
                   [AC_TRY_COMPILE([], [],
                                   [ww_cv_cc_finline_functions="yes"
                                    if test -s conftest.err ; then
                                        for i in unused 'not supported' ; do
                                            if $GREP -iq "$i" conftest.err; then
                                                ww_cv_cc_finline_functions="no"
                                                break;
                                            fi
                                        done
                                    fi
                                   ...
                                   [ww_cv_cc_finline_functions="no"])])
        if test "$ww_cv_cc_finline_functions" = "yes" ; then
        ...
        AC_CACHE_CHECK([if $CC supports -fno-strict-aliasing],
                   [ww_cv_cc_fno_strict_aliasing],
                   [AC_TRY_COMPILE([], [],
                                   [ww_cv_cc_fno_strict_aliasing="yes"],
                                   [ww_cv_cc_fno_strict_aliasing="no"])])
        if test "$ww_cv_cc_fno_strict_aliasing" = "yes" ; then
    ...
    if test ! -z "$RESTRICT_CFLAGS" ; then
        ...
        AC_CACHE_CHECK([if $CC supports $RESTRICT_CFLAGS],
                   [ww_cv_cc_restrict_cflags],
                   [AC_TRY_COMPILE([], [],
                                   [ww_cv_cc_restrict_cflags="yes"],
                                   [ww_cv_cc_restrict_cflags="no"])])
        if test "$ww_cv_cc_restrict_cflags" = "yes" ; then
            add=" $RESTRICT_CFLAGS"
        fi
    ...
    AC_CACHE_CHECK([if $CC supports __builtin_expect],
        [ww_cv_cc_supports___builtin_expect],
        [AC_TRY_LINK([],
          [void *ptr = (void*) 0;
           if (__builtin_expect (ptr != (void*) 0, 1)) return 0;],
          [ww_cv_cc_supports___builtin_expect="yes"],
          [ww_cv_cc_supports___builtin_expect="no"])])
    if test "$ww_cv_cc_supports___builtin_expect" = "yes" ; then
        have_cc_builtin_expect=1
    else
        have_cc_builtin_expect=0
    fi
    AC_DEFINE_UNQUOTED([WW_C_HAVE_BUILTIN_EXPECT], [$have_cc_builtin_expect],
        [Whether C compiler supports __builtin_expect])

    # see if the C compiler supports __builtin_prefetch
    AC_CACHE_CHECK([if $CC supports __builtin_prefetch],
        [ww_cv_cc_supports___builtin_prefetch],
        [AC_TRY_LINK([],
          [int ptr;
           __builtin_prefetch(&ptr,0,0);],
          [ww_cv_cc_supports___builtin_prefetch="yes"],
          [ww_cv_cc_supports___builtin_prefetch="no"])])
    if test "$ww_cv_cc_supports___builtin_prefetch" = "yes" ; then
        have_cc_builtin_prefetch=1
    else
        have_cc_builtin_prefetch=0
    fi
    AC_DEFINE_UNQUOTED([WW_C_HAVE_BUILTIN_PREFETCH], [$have_cc_builtin_prefetch],
        [Whether C compiler supports __builtin_prefetch])

    # see if the C compiler supports __builtin_clz
    AC_CACHE_CHECK([if $CC supports __builtin_clz],
        [ww_cv_cc_supports___builtin_clz],
        [AC_TRY_LINK([],
            [int value = 0xffff; /* we know we have 16 bits set */
             if ((8*sizeof(int)-16) != __builtin_clz(value)) return 0;],
            [ww_cv_cc_supports___builtin_clz="yes"],
            [ww_cv_cc_supports___builtin_clz="no"])])
    if test "$ww_cv_cc_supports___builtin_clz" = "yes" ; then
        have_cc_builtin_clz=1
    else
        have_cc_builtin_clz=0
    fi
    AC_DEFINE_UNQUOTED([WW_C_HAVE_BUILTIN_CLZ], [$have_cc_builtin_clz],
    ...
    WW_ENSURE_CONTAINS_OPTFLAGS("$WW_CFLAGS_BEFORE_PICKY")
    WW_CFLAGS_BEFORE_PICKY="$co_result"

    AC_MSG_CHECKING([for C optimization flags])
    WW_ENSURE_CONTAINS_OPTFLAGS(["$CFLAGS"])
    ...
    WW_CHECK_IDENT([CC], [CFLAGS], [c], [C])

])


AC_DEFUN([_WW_START_SETUP_CC],[
    ww_show_subtitle "C compiler and preprocessor"
...
AC_DEFUN([_WW_PROG_CC],[
    #
    # Check for the compiler
    #
    WW_VAR_SCOPE_PUSH([ww_cflags_save dummy ww_cc_arvgv0])
    ww_cflags_save="$CFLAGS"
    AC_PROG_CC
    BASECC="`basename $CC`"
    CFLAGS="$ww_cflags_save"
    AC_DEFINE_UNQUOTED(WW_CC, "$CC", [Warewulf underlying C compiler])
    set dummy $CC
    ww_cc_argv0=[$]2
    WW_WHICH([$ww_cc_argv0], [WW_CC_ABSOLUTE])
    AC_SUBST(WW_CC_ABSOLUTE)
    WW_VAR_SCOPE_POP
])
