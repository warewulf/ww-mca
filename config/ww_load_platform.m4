...
AC_DEFUN([WW_LOAD_PLATFORM], [
    ...
    if test "$with_platform" = "yes" ; then
        ...
        WW_LOG_MSG([Loading environment file $with_platform, with contents below])
        WW_LOG_FILE([$with_platform])
        ...
        if test "$WW_PLATFORM_LOADED" != "" ; then
           platform_loaded="$WW_PLATFORM_LOADED"
        else
           platform_loaded="$with_platform"
        fi
        echo "Loaded platform arguments for $platform_loaded"
        WW_LOG_MSG([Loaded platform arguments for $platform_loaded])
