...
# Minimum versions
my $ww_automake_version = "1.12.2";
my $ww_autoconf_version = "2.69";
my $ww_libtool_version = "2.4.2";

# Search paths
my $ww_autoconf_search = "autoconf";
my $ww_automake_search = "automake";
my $ww_libtoolize_search = "libtoolize;glibtoolize";
...
sub ignored {
    ...
    # If this directory does not have .ww_ignore, or if it has a
    # .ww_unignore that has my username in it, then add it to the
    # list of components.
    my $ignored = 0;
    ...
    if (-f "$dir/.ww_unignore") {
        open(UNIGNORE, "$dir/.ww_unignore") ||
            my_die "Can't open $dir/.ww_unignore file";
        my $unignore;
        $unignore .= $_
            while (<UNIGNORE>);
        close(UNIGNORE);
    ...
    if (exists($exclude_list->{$framework}) &&
        ...
        if (-d $dir) {
            ...
            foreach my $d (readdir(DIR)) {
                ...
                verbose "--- Found Warewulf / $framework / $d component: src/mca/$framework/$d\n";
                ...
                # Check ignore status
                if (ignored("$dir/$d")) {
                    verbose "    => Ignored (found .ww_ignore file)\n";
                } else {
...
sub mca_process_project {
    ...
    foreach my $d (@my_dirs) {
        ...
        if ("common" eq $d ||
            ...
            verbose "\n=== Found Warewulf framework: src/mca/$d\n";
...
dnl Frameworks in the Warewulf project and their corresponding directories
m4_define([mca_ww_framework_list], [$frameworks_comma])
...
";
    ...
    foreach my $f (@mykeys) {
        ...
        $m4 .= "dnl Components in the Warewulf / $f framework
m4_define([mca_ww_${f}_m4_config_component_list], [$m4_config_component_list])
m4_define([mca_ww_${f}_no_config_component_list], [$no_config_component_list])
...
tools:
    ...
    GNU Autoconf: $ww_autoconf_version
    GNU Automake: $ww_automake_version
    GNU Libtool: $ww_libtool_version
=================================================================\n";
    my_exit(1);
}
...
my $project_name_long = "Warewulf";
my $project_name_short = "ww";
...
# Verify that we're in the Warewulf root directory by checking for a token file.

my_die "Not at the root directory of an Warewulf source tree"
    if (! -f "config/ww_mca.m4");

my_die "autogen.pl has been invoked in the source tree of a Warewulf distribution tarball; aborting...
...
my $step = 1;
verbose "Warewulf autogen (buckle up!)

$step. Checking tool versions\n\n";

# Check the autotools revision levels
&find_and_check("autoconf", $ww_autoconf_search, $ww_autoconf_version);
&find_and_check("libtool", $ww_libtoolize_search, $ww_libtool_version);
&find_and_check("automake", $ww_automake_search, $ww_automake_version);
...
# Figure out if we're at the top level of the Warewulf tree or not.
if (! (-f "VERSION" && -f "configure.ac" && -f $topdir_file)) {
    print("\n\nYou must run this script from the top-level directory of the Warewulf tree.\n\n");
    my_exit(1);
}
...
verbose "
================================================
Warewulf autogen: completed successfully.  w00t!
================================================\n\n";
